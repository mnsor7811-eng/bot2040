...
# ذاكرة سريعة لتخزين الأسعار مؤقتاً لتفادي البُطء
PRICES_CACHE = {}
CACHE_LAST_UPDATE = {}
CACHE_DURATION = 600  # تحديث الأسعار كل 10 دقائق تلقائياً
...
def update_prices_cache(server_id, service_code):
    """جلب قائمة الأسعار وحفظها في الذاكرة لتكون سرعة الاستجابة فلاحية"""
    key = f"{server_id}_{service_code}"
    current_time = time.time()
    
    # إذا كانت البيانات مخزنة ولم يمر عليها 10 دقائق، لا تقم بالاتصال مجدداً
    if key in PRICES_CACHE and (current_time - CACHE_LAST_UPDATE.get(key, 0)) < CACHE_DURATION:
    ...
    try:
        ...
        PRICES_CACHE[key] = res
        CACHE_LAST_UPDATE[key] = current_time
        return res
    except:
...
def countries_keyboard_fast(server_id, service_code):
    markup = InlineKeyboardMarkup()
    prices_data = update_prices_cache(server_id, service_code)
    ...
    for code, name in POPULAR_COUNTRIES:
        ...
        if code in prices_data and service_code in prices_data[code]:
            ...
            final_price = round(cost * (1 + PROFIT_MARGIN), 2)
            price_str = f"${final_price}"
        
        # عرض اسم الدولة مع سعرها الفعلي الحقيقي فوراً
        markup.add(InlineKeyboardButton(f"{name} - {price_str}", callback_data=f"b_{server_id}_{service_code}_{code}"))
...
def callback_handler(call):
    ...
    if call.data == "back_main":
        ...
        prices_data = update_prices_cache(server_id, srv_code)
        price = 0.50
        if country_code in prices_data and srv_code in prices_data[country_code]:
            cost = float(prices_data[country_code][srv_code]['cost'])
        ...
        if "ACCESS_NUMBER" in res:
            ...
            conn.close()
            bot.send_message(chat_id, "❌ **تنبيه:** لا توجد أرقام متوفرة حالياً لهذه الدولة أو رصيدك في حساب السيرفر غير كافٍ.")
