...
DEFAULT_PRICE = 0.50  # سعر افتراضي في حال عدم استجابة الـ API مؤقتاً
...
def fetch_server_prices(server_id, service_code):
    ...
    try:
        ...
        # استخراج الأسعار من رد JSON المقبول في Grizzly و Tiger
        for c_code, s_data in res.items():
            if service_code in s_data:
                cost = float(s_data[service_code]['cost'])
                prices[str(c_code)] = round(cost * (1 + PROFIT_MARGIN), 2)
    except:
        pass
...
def countries_keyboard_fast(server_id, service_code):
    ...
    for code, name in POPULAR_COUNTRIES:
        # إذا وجد السعر يتم عرضه، وإن لم يجد يظهر السعر الافتراضي حتى لا يتعطل الزر
...
def callback_handler(call):
    ...
    if call.data == "back_main":
        ...
        # اختيار السعر الدقيق أو الافتراضي لحماية الشراء
        ...
        if "ACCESS_NUMBER" in res:
            ...
            conn.close()
            bot.send_message(chat_id, "❌ **تنبيه:** الأرقام غير متوفرة حالياً لهذه الدولة في السيرفر، جرب دولة أخرى.")
        elif "NO_BALANCE" in res:
            conn.close()
            bot.send_message(chat_id, "⚠️ **تنبيه الإدارة:** رصيد الحساب الرئيسي في موقع السيرفر غير كافٍ لشراء الرقم.")
        else:
            conn.close()
