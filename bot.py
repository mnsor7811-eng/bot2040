...
# قاموس شامل لكافة أكواد الدول (بما فيها الأكواد المرتفعة 100+)
COUNTRY_MAP = {
    ...
    "161": {"name": "اليمن", "flag": "🇾🇪"}, "162": {"name": "اليمن", "flag": "🇾🇪"},
    "163": {"name": "السعودية", "flag": "🇸🇦"}, "164": {"name": "الإمارات", "flag": "🇦🇪"},
    "165": {"name": "مصر", "flag": "🇪🇬"}, "166": {"name": "العراق", "flag": "🇮🇶"},
    "167": {"name": "الأردن", "flag": "🇯🇴"}, "168": {"name": "المغرب", "flag": "🇲🇦"},
    "169": {"name": "مدغشقر", "flag": "🇲🇬"}, "170": {"name": "الجزائر", "flag": "🇩🇿"},
    "171": {"name": "تونس", "flag": "🇹🇳"}, "172": {"name": "البرازيل", "flag": "🇧🇷"},
    "174": {"name": "تركيا", "flag": "🇹🇷"}, "175": {"name": "أمريكا", "flag": "🇺🇸"},
    "176": {"name": "كندا", "flag": "🇨🇦"}, "177": {"name": "فرنسا", "flag": "🇫🇷"},
    "179": {"name": "ألمانيا", "flag": "🇩🇪"}, "187": {"name": "أمريكا", "flag": "🇺🇸"}
}
...
# ==================== 5. معالجة الأحداث والأزرار ====================
@bot.message_handler(commands=['start'])
def start_cmd(message):
    user_id = message.from_user.id
    if is_user_banned(user_id):
        bot.send_message(message.chat.id, "🚫 عذراً، الحساب محظور.")
        return

    user_data = get_or_create_user(user_id, message.from_user.first_name or "المستخدم")
...
def callback_handler(call):
    ...
    if call.data == "back_main":
        ...
        markup.add(InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="back_main"))
        bot.edit_message_text("💳 اختر طريقة الشحن المناسبة:", chat_id, message_id, reply_markup=markup)
        ...
        text = f"📌 **تفاصيل الشحن عبر {info['name']}**:\n\n" \
               ...
               f"أرسل إشعار التحويل للدعم الفني لتمويل حسابك: {ADMIN_USERNAME}"
        bot.edit_message_text(text, chat_id, message_id, parse_mode="Markdown", reply_markup=back_button())

    elif call.data == "my_account":
        user_data = get_or_create_user(user_id, call.from_user.first_name)
        text = f"👤 **معلومات حسابك**:\n\nالاسم: {user_data[1]}\nالآيدي: `{user_data[0]}`\nالرصيد: ${user_data[2]:.2f}\nأسئلة الذكاء الاصطناعي: {user_data[3]}"
        bot.edit_message_text(text, chat_id, message_id, parse_mode="Markdown", reply_markup=back_button())

    elif call.data == "smm_main":
        bot.edit_message_text("🚀 **خدمات الرشق SMM**:\n\nيرجى اختيار الخدمة أو التواصل مع الدعم الفني.", chat_id, message_id, reply_markup=back_button())

    elif call.data == "ai_landing":
        bot.edit_message_text("🤖 **قسم الذكاء الاصطناعي**:\n\nيمكنك إرسال استفسارك مباشرة للبوت.", chat_id, message_id, reply_markup=back_button())

    elif call.data in ["support", "transfer", "best_selling", "most_available", "fast_buy_tg", "fast_buy_wa"]:
        bot.edit_message_text(f"🎧 للتواصل المباشر مع الدعم الفني: {ADMIN_USERNAME}", chat_id, message_id, reply_markup=back_button())
    ...
    # --- قسم شراء الأرقام ---
        ...
        markup.add(InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="back_main"))
        bot.edit_message_text("📞 اختر السيرفر الشغال:", chat_id, message_id, reply_markup=markup)
        ...
        markup.add(InlineKeyboardButton("🔙 العودة للسيرفرات", callback_data="buy_number"))
        ...
        if "ACCESS_NUMBER" in res:
            ...
            conn.close()
            bot.send_message(chat_id, "⚠️ الأرقام المتاحة لهذه الدولة نفدت حالياً لدى المزود، جرب اختيار دولة أخرى.")
        else:
            conn.close()
            bot.send_message(chat_id, f"❌ تعذر الطلب: {res}")
        ...
        if "STATUS_OK" in res:
            code = res.split(":")[1]
            bot.send_message(chat_id, f"🎉 **كود التفعيل الخاص بك**: `{code}`\n\n(اضغط على الكود لنسخه)", parse_mode="Markdown")
        else:
            bot.answer_callback_query(call.id, "⏳ لم يصل الكود بعد، أدخل الرقم في التطبيق ثم حاول مرة أخرى.", show_alert=True)
        ...
        if purchase and purchase[2] == 'PENDING':
            ...
            if "ACCESS_NUMBER" in res:
                ...
                conn.close()
                bot.edit_message_text(f"⚠️ تم إلغاء الرقم وإرجاع (${cost:.2f}) لحسابك.\nنفدت الأرقام لهذه الدولة لدى المزود حالياً.", chat_id, message_id, reply_markup=back_button())
        else:
            conn.close()
