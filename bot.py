...
TOKEN = '8927305428:AAFSExA5-PlPNbMFin5WPmOTGdv2vuQ_66Q'
...
def callback_handler(call):
    ...
    if call.data == "back_main":
          # معالجة أزرار طرق الدفع بشكل مستقل ومنع تعليق الأزرار
    elif call.data.startswith("pay_"):
        pay_key = call.data.replace("pay_", "")
        
        # بيانات المحافظ وطرق الدفع (يمكنك تعديلها حسب رغبتك)
        PAYMENT_DETAILS = {
            'kuraimi': {'name': '🏦 بنك الكريمي', 'acc': '3134706987', 'min': '100 ريال', 'rate': '1$ = 550 ريال'},
            'jeeb': {'name': '📱 محفظة جيب', 'acc': '374468', 'min': '50 ريال', 'rate': '1$ = 550 ريال'},
            'onecash': {'name': '💳 محفظة ون كاش', 'acc': '140601836', 'min': '100 ريال', 'rate': '1$ = 550 ريال'},
        ...
        if pay_info:
            msg = (f"{pay_info['name']}\n\n"
                   ...
                   f"2. قم بإرسال صورة إشعار التحويل (الوصل) ومعرف حسابك إلى حساب الإدارة:\n"
                   f"👤 الإدارة: @Num_s7\n\n"
    ...
    # === [تم الإضافة والتعديل هنا لمعالجة أزرار طرق الشحن] ===
    elif call.data.startswith("pay_"):
        pay_key = call.data.replace("pay_", "")
        pay_info = PAYMENT_DETAILS.get(pay_key)
