...
# جلب كافة الدول المتاحة من API السيرفر ديناميكياً
    ...
    try:
        ...
        for country_code, data in res.items():
            if service_code in data:
                cost = float(data[service_code]['cost'])
                count_available = data[service_code].get('count', 0)
                
                # إظهار الدول التي تحتوي أرقام متوفرة فقط
                if count_available > 0:
                    final_price = round(cost * (1 + PROFIT_MARGIN), 2)
                    markup.add(InlineKeyboardButton(f"🌐 دولة {country_code} - ${final_price} ({count_available} متوفر)", 
                                                 callback_data=f"buy_num_{server_id}_{service_code}_{country_code}_{final_price}"))
                    count += 1
                    if count >= 30: # تحديد حد أقصى لحجم القائمة كي لا تتجاوز حدود تليجرام
                        break
    except:
        pass
...
def callback_handler(call):
    ...
    if call.data == "back_main":
        ...
        text = f"🌐 **جاري جلب كافة الدول المتاحة لخدمة ({srv_code.upper()})...**"
        ...
        if "ACCESS_NUMBER" in res:
            ...
            conn.close()
            bot.send_message(chat_id, "❌ **تنبيه:** لا توجد أرقام متاحة حالياً لهذه الدولة أو رصيدك في سيرفر الموقع نافد.")
