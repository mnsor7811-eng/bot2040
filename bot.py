...
# ==================== 1. الإعدادات الأساسية (تم دمج التوكنات) ====================
...
def ai_section_keyboard():
    ...
    btn_buy_pkg = InlineKeyboardButton("💳 شحن رصيد / شراء باقة", callback_data="recharge")
...
def callback_handler(call):
    ...
    # ---------------- قسم الذكاء الاصطناعي التسويقي ----------------
        ...
        promo_text = (
            ...
            f"💎 **باقات واشتراكات الذكاء الاصطناعي المتاحة:**\n"
        ...
        bot.edit_message_text(promo_text, chat_id, message_id, parse_mode="Markdown", reply_markup=ai_section_keyboard())

    elif call.data == "ask_ai_direct":
        user_data = get_or_create_user(user_id, call.from_user.first_name)
        ai_balance = user_data[3]
        if ai_balance > 0:
            msg = bot.send_message(chat_id, f"🧠 **جاهز لاستقبال سؤالك!**\nارسل أي سؤال أو نص تريد صياغته الآن وسأجيبك فوراً:")
    ...
    # ---------------- قسم شحن الرصيد التفاعلي ----------------
    elif call.data == "recharge":
        text = (f"💳 **قسم شحن الرصيد والاشتراكات:**\n\n"
                f"شحن رصيدك يتيح لك شراء الأرقام الافتراضية واشتراكات الذكاء الاصطناعي فوراً.\n\n"
                f"🏛️ **طرق الدفع والتحويل المتاحة:**\n"
                f"▪️ **بنك الكريمي:** `3134706987`\n"
                f"▪️ **محفظة جيب:** `374468`\n"
                f"▪️ **ون كاش:** `140601836`\n"
                f"▪️ **بايننس (Binance Pay / ID):** `979808293`\n\n"
                f"📩 **طريقة التفعيل:**\n"
                f"بعد التحويل، قم بتصوير إشعار التحويل وإرساله إلى الدعم الفني مرفقاً برقم الأيدي الخاص بك (`{user_id}`):\n"
                f"👉 **معرف التواصل:** {ADMIN_USERNAME}")
...
def process_ai_question(message):
    ...
    if user and user[0] > 0:
        ...
        try:
            ...
            bot.send_message(message.chat.id, f"🤖 **الرد:**\n\n{reply}", parse_mode="Markdown", reply_markup=back_ai)
...
def process_admin_sub_ai(message):
    try:
        ...
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute('UPDATE users SET ai_balance = ai_balance - 1 WHERE user_id = ?', (target_id,))
        conn.commit()
        conn.close()
