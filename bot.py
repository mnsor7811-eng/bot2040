...
REWARD_PER_INVITE = 0.05  # مكافأة الدعوة
MIN_TRANSFER_AMOUNT = 1.0  # الحد الأدنى للتحويل
...
# ==================== 2. قاموس كودات ومفاتيح الدول الشامل ====================
COUNTRY_DATA = {
    # Grizzly IDs
    ...
    "61": {"name": "السنغال", "flag": "🇸🇳"}, "62": {"name": "تركيا", "flag": "🇹🇷"},
    "63": {"name": "تونس", "flag": "🇹🇳"}, "64": {"name": "فرنسا", "flag": "🇫🇷"},
    "65": {"name": "فنلندا", "flag": "🇫🇮"}, "66": {"name": "السودان", "flag": "🇸🇩"},
    "67": {"name": "أستراليا", "flag": "🇦🇺"}, "68": {"name": "بلجيكا", "flag": "🇧🇪"},
    "73": {"name": "البرازيل", "flag": "🇧🇷"}, "86": {"name": "إيطاليا", "flag": "🇮🇹"},
    ...
    # Extended Calling Codes Range (المجال المترجم للكودات الممتدة من 106 إلى 200)
    "176": {"name": "غينيا بيساو", "flag": "🇬🇼"},
    "177": {"name": "سيراليون", "flag": "🇸🇱"},
    "178": {"name": "الصومال", "flag": "🇸🇴"},
    "179": {"name": "موزمبيق", "flag": "🇲🇿"},
    "180": {"name": "زيمبابوي", "flag": "🇿🇼"},
    "181": {"name": "بوتسوانا", "flag": "🇧🇼"},
    "182": {"name": "ناميبيا", "flag": "🇳🇦"},
    "183": {"name": "ملاوي", "flag": "🇲🇼"},
    "184": {"name": "ليسوتو", "flag": "🇱🇸"},
    "185": {"name": "إسواتيني", "flag": "🇸🇿"},
    "186": {"name": "مدغشقر", "flag": "🇲🇬"},
    "187": {"name": "زومبيا", "flag": "🇿🇲"},
    "188": {"name": "أنغولا", "flag": "🇦🇴"}
}

def get_country_info(code_str):
    code_str = str(code_str).strip()
    if code_str in COUNTRY_DATA:
        return COUNTRY_DATA[code_str]['name'], COUNTRY_DATA[code_str]['flag']
...
def main_keyboard(user_id):
    ...
    markup.row(InlineKeyboardButton("📞 شراء رقم افتراضي", callback_data="select_server_menu"))
...
def server_selection_keyboard():
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("🐻 سيرفر Grizzly SMS (الأساسي)", callback_data="select_server_grizzly"))
...
def services_keyboard(server_id):
    ...
    markup.add(InlineKeyboardButton("🔙 العودة لاختيار السيرفر", callback_data="select_server_menu"))
    return markup
...
def countries_keyboard_fast(server_id, service_code, page=0):
    ...
    for code, price in items:
        c_name, flag = get_country_info(code)
        button_text = f"{c_name} {flag} : ${price:.2f}"
...
def add_balance_cmd(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ الاستخدام: /add_bal USER_ID AMOUNT")
...
def start_cmd(message):
    ...
    if not user:
        ...
        if ref_id != 0:
            ...
            conn.commit()
            try:
                bot.send_message(ref_id, f"🎉 انضم شخص جديد عبر رابطك!\n🎁 تم إضافة ${REWARD_PER_INVITE:.2f} إلى رصيدك.")
            except:
                pass
    conn.close()
...
def callback_handler(call):
    ...
    if call.data == "back_main":
    ...
    elif call.data == "select_server_menu":
        bot.edit_message_text("🌐 **قسم السيرفرات المتاحة**\n\nاختر السيرفر الذي ترغب بالشراء منه:", chat_id, message_id, reply_markup=server_selection_keyboard(), parse_mode="Markdown")

    elif call.data == "select_server_grizzly":
        bot.edit_message_text("⚙️ **سيرفر Grizzly SMS**\n\nاختر التطبيق المراد تفعيله:", chat_id, message_id, reply_markup=services_keyboard("grizzly"), parse_mode="Markdown")

    elif call.data == "recharge_menu":
        text = "🎳 **قسم شحن الرصيد / الاشتراكات**\n\nاختر وسيلة الدفع التي تناسبك من القائمة أدناه:"
        bot.edit_message_text(text, chat_id, message_id, reply_markup=recharge_keyboard(), parse_mode="Markdown")
        ...
        if pay_info:
            msg = (f"*{pay_info['name']}*\n\n"
                   ...
                   f"⚠️ **خطوات الشحن**:\n"
                   f"1. قم بالتحويل إلى الرقم الموضح أعلاه.\n"
                   f"2. أرسل صورة إشعار التحويل مع آيدي حسابك [`{user_id}`] إلى الدعم:\n"
                   f"👤 **الإدارة**: {ADMIN_USERNAME}")
            back_mk = InlineKeyboardMarkup()
            back_mk.add(InlineKeyboardButton("🔙 العودة لوسائل الدفع", callback_data="recharge_menu"))
            bot.edit_message_text(msg, chat_id, message_id, reply_markup=back_mk, parse_mode="Markdown")
        ...
        if "ACCESS_NUMBER" in res:
            ...
            conn.close()
            bot.send_message(chat_id, f"❌ تعذر الشراء حالياً. رد السيرفر: {res}")
        ...
        parts = call.data.split("_")
        server_id, old_tz_id = parts[2], parts[3]
        srv = SERVERS.get(server_id)
        
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute('SELECT user_id, cost, status, service, country_code FROM purchases WHERE tz_id = ?', (old_tz_id,))
        ...
        if purchase and purchase[2] == 'PENDING':
            ...
            # إلغاء وإرجاع الرصيد
            grizzly_request({'action': 'setStatus', 'status': '8', 'id': old_tz_id}, srv['api_key'], srv['url'])
            cursor.execute('UPDATE purchases SET status = "CANCELLED" WHERE tz_id = ?', (old_tz_id,))
            cursor.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', (cost, user_id))
            conn.commit()
            
            try:
                bot.answer_callback_query(call.id, text="🔄 جاري جلب رقم جديد لنفس الدولة...", show_alert=False)
            except:
                pass
            
            # طلب رقم جديد بنفس المعايير
            res = grizzly_request({'action': 'getNumber', 'service': srv_code, 'country': country_code}, srv['api_key'], srv['url'])
            ...
            if "ACCESS_NUMBER" in res:
                ...
                cursor.execute('UPDATE users SET balance = balance - ? WHERE user_id = ?', (cost, user_id))
                cursor.execute('INSERT INTO purchases (user_id, tz_id, phone, service, cost, country_code) VALUES (?, ?, ?, ?, ?, ?)',
                               (user_id, new_tz_id, formatted_phone, srv_code, cost, country_code))
                conn.commit()
                conn.close()
                ...
                msg = (f"🆔 **رقم الطلب الجديد** : `{new_tz_id}`\n"
                       ...
                       f"💵 **السعر** : `${cost:.2f}`\n\n"
                       f"📋 *اضغط على الرقم لنسخه فوراً*")
                bot.edit_message_text(msg, chat_id, message_id, parse_mode="Markdown", reply_markup=active_number_keyboard(new_tz_id, server_id, formatted_phone, srv_code))
            else:
                conn.close()
                bot.send_message(chat_id, "❌ لم يتوفر رقم جديد لنفس الدولة حالياً، تم إرجاع الرصيد لحسابك.")
        else:
            conn.close()
            try:
                bot.answer_callback_query(call.id, text="⚠️ الطلب ملغى أو منتهي مسبقاً", show_alert=True)
            except:
                pass
        ...
        if purchase and purchase[1] == 'PENDING':
            ...
            conn.close()
            try:
                bot.answer_callback_query(call.id, text="❌ تم إلغاء الرقم واستعادة الرصيد", show_alert=False)
            except:
                pass
            bot.edit_message_text(f"❌ تم إلغاء الطلب بنجاح وإعادة مبلغ (${cost:.2f}) إلى رصيدك.", chat_id, message_id, reply_markup=back_button())
        else:
            conn.close()
            try:
                bot.answer_callback_query(call.id, text="العملية ملغاة مسبقاً", show_alert=True)
            except:
                pass
        ...
        msg = (f"💎 **نظام اربح رصيد مجاني** 💎\n\n"
               f"شارك رابط الدعوة مع أصدقائك واحصل على رصيد مجاني عن كل شخص ينضم!\n\n"
               f"🎁 المكافأة لكل شخص: `${REWARD_PER_INVITE:.2f}`\n"
               f"👥 الدعوات الناجحة: {invite_count} شخص\n"
        ...
        text = ("🔄 **تحويل الرصيد بين الحسابات**\n\n"
                "💵 الحد الأدنى للتحويل: $1.00\n"
                "✨ العمولة: 0%\n\n"
                "📌 **أرسل آيدي (User ID) الشخص المراد التحويل إليه الآن:**")
        bot.edit_message_text(text, chat_id, message_id, parse_mode="Markdown", reply_markup=back_button())

    elif call.data == "smm_main":
        bot.edit_message_text("🚀 قسم خدمات الرشق وشحن الألعاب\n\nاختر الخدمة التي تود طلبها:", chat_id, message_id, reply_markup=back_button())

    elif call.data == "ai_landing":
        bot.edit_message_text("🤖 قسم الذكاء الاصطناعي\n\nاكتب سؤالك فوراً في الدردشة وسأجيبك فوراً.", chat_id, message_id, reply_markup=back_button())
        ...
        bot.edit_message_text("🔥 الأكثر مبيعاً:\n1. Grizzly - تليجرام روسيا\n2. Grizzly - واتساب أمريكا", chat_id, message_id, reply_markup=back_button())

    elif call.data == "most_available":
        bot.edit_message_text("🎲 الأكثر توفراً: أرقام روسيا وأمريكا والسنغال متوفرة بكثرة.", chat_id, message_id, reply_markup=back_button())

    elif call.data == "support":
        bot.edit_message_text(f"🎧 الدعم الفني المباشر:\n{ADMIN_USERNAME}", chat_id, message_id, reply_markup=back_button())
        ...
        conn.close()
        bot.edit_message_text(f"✔ إحصائيات الشراء الناجح:\nتم تفعيل أكثر من {count + 150} رقم بنجاح!", chat_id, message_id, reply_markup=back_button())
        ...
        msg = (f"👤 **معلومات حسابك**:\n\n"
               ...
               f"🤖 **رصيد الذكاء الاصطناعي**: {user_data[3]} أسئلة")
        bot.edit_message_text(msg, chat_id, message_id, parse_mode="Markdown", reply_markup=back_button())

    elif call.data == "other_services":
        bot.edit_message_text("🛸 ميزات إضافية قادمة قريباً.", chat_id, message_id, reply_markup=back_button())
...
def handle_user_steps(message):
    ...
    if step_data['step'] == 'TRANSFER_TARGET':
        ...
        bot.send_message(message.chat.id, f"👤 المستلم: {target_user[0]} ({target_id})\n\nأرسل المبلغ المراد تحويله بالدولار ($1.00 كحد أدنى):")

    elif step_data['step'] == 'TRANSFER_AMOUNT':
        try:
            amount = float(message.text.strip())
        except ValueError:
            bot.send_message(message.chat.id, "❌ يرجى كتابة المبلغ بالأرقام فقط.")
            return
        ...
        target_id, target_name = step_data['target_id'], step_data['target_name']
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute('SELECT balance FROM users WHERE user_id = ?', (user_id,))
        ...
        del USER_STEPS[user_id]
        bot.send_message(message.chat.id, f"✅ تم تحويل ${amount:.2f} بنجاح إلى {target_name} ({target_id})", reply_markup=back_button())
        try:
...
def handle_ai_chat(message):
    ...
    if res and res[0] > 0:
        ...
        try:
            ...
            bot.reply_to(message, "⚠️ حدث خطأ في الاتصال بنموذج الذكاء الاصطناعي.")
    else:
        conn.close()
        bot.reply_to(message, "❌ نفد رصيدك المجاني لاستخدام الذكاء الاصطناعي.")
