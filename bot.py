...
# ==================== دالة استخراج معلومات الدولة الذكية (المحدثة للتكرار والخطوط) ====================
def get_clean_country_info(code_str):
    ...
    # 2. تنظيف أكواد السيرفرات الطويلة والخماسية (مثل 10056 أو 10015)
    if len(code_str) >= 4:
    ...
    # 3. معالجة الأكواد الترتيبية/الخطوط الحسابية
    if code_str.isdigit():
...
def countries_keyboard_fast(server_id, service_code, page=0):
    ...
    # معالجة تكرار الدول لحساب الأرقام المكررة (إسبانيا، إسبانيا 2، إسبانيا 3)
    country_counts = {}
    formatted_list = []
