...
# قائمة أهم الدول الأكثر طلباً لتسريع الأداء (رمز الدولة + الاسم + العلم)
POPULAR_COUNTRIES = [
    ("0", "🇷🇺 روسيا"),
    ("187", "🇺🇸 أمريكا"),
    ("16", "🇬🇧 بريطانيا"),
...
def countries_keyboard_fast(server_id, service_code):
    ...
    for code, name in POPULAR_COUNTRIES:
        price = 0.50 # سعر افتراضي سرعةً للأداء
...
def callback_handler(call):
    ...
    if call.data == "back_main":
        ...
        if "ACCESS_NUMBER" in res:
            ...
            conn.close()
            bot.send_message(chat_id, "❌ **تنبيه:** لا توجد أرقام متاحة حالياً لهذه الدولة أو رصيدك في حساب الموقع نافد.")
