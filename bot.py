...
REWARD_PER_INVITE = 0.05  # قيمة مكافأة الدعوة لكل شخص جديد ($0.05)
...
def add_balance_cmd(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ **طريقة الاستخدام الخاطئة!**\nارسل الأمر كالتالي:\n`/add_bal USER_ID AMOUNT`\n\nمثال لشحن $10 لنفسك:\n`/add_bal 6113734300 10`", parse_mode="Markdown")

# ==================== 6. معالجة الأوامر والطلب والإحالة ====================
...
def start_cmd(message):
    ...
    if not user:
        ...
        # تقديم مكافأة للشخص الموصي إذا وجد
        if ref_id != 0:
...
def callback_handler(call):
    ...
    # === قسم الربح المجاني (الإحالة) ===
        ...
        msg = (f"💎 **نظام اربح رصيد مجاني** 💎\n\n"
               f"قم بنشر رابط الإحالة الخاص بك بين أصدقائك أو في المجموعات، واحصل على رصيد مجاني في البوت لكل شخص ينضم عبر رابطك!\n\n"
    ...
    # === معالجة باقي الأزرار الرئيسية ===
        ...
        bot.send_message(chat_id, "🔄 **تحويل الرصيد:** تواصل مع الدعم الفني لتحويل الرصيد بين الحسابات.")
...
def handle_user_steps(message):
    ...
    if step_data['step'] == 'WAITING_LINK':
