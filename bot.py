...
# ==================== قاموس دول Grizzly SMS الرسمي والمدقق ====================
GRIZZLY_COUNTRIES = {
    "0": {"name": "روسيا", "flag": "🇷🇺"},
    ...
    "64": {"name": "فرنسا", "flag": "🇫🇷"},
    "65": {"name": "فنلندا", "flag": "🇫🇮"},
    "66": {"name": "السودان", "flag": "🇸🇩"},
    "67": {"name": "أستراليا", "flag": "🇦🇺"},
    "68": {"name": "بلجيكا", "flag": "🇧🇪"},
...
def get_country_info(code_str):
    ...
    if code_str in GRIZZLY_COUNTRIES:
        return GRIZZLY_COUNTRIES[code_str]['name'], GRIZZLY_COUNTRIES[code_str]['flag']
...
def callback_handler(call):
    ...
    elif call.data in ["buy_number", "select_server_grizzly"]:
        text = "⚙️ **سيرفر Grizzly SMS**\n\nاختر التطبيق المراد تفعيله:"
        bot.edit_message_text(text, chat_id, message_id, reply_markup=services_keyboard("grizzly"), parse_mode="Markdown")
    ...
    # ==================== تصحيح دالة تغيير الرقم للنفس الدولة حصراً ====================
        ...
        if purchase and purchase[2] == 'PENDING':
            ...
            # 1. إلغاء الرقم القديم من السيرفر وفي قاعدة البيانات
            ...
            bot.answer_callback_query(call.id, text="🔄 جاري إلغاء الرقم وجلب رقم جديد لنفس الدولة...", show_alert=False)
            
            # 2. طلب رقم جديد فوراً لنفس الدولة والتطبيق
            res = grizzly_request({'action': 'getNumber', 'service': srv_code, 'country': country_code}, srv['api_key'], srv['url'])
            ...
            if "ACCESS_NUMBER" in res:
                ...
                conn.close()
                bot.send_message(chat_id, "❌ لم يتوفر رقم آخر لنفس الدولة حالياً، تم إرجاع المبلغ لرصيدك.")
        else:
            conn.close()
            bot.answer_callback_query(call.id, text="⚠️ الطلب منتهي أو ملغى مسبقاً", show_alert=True)
...
def handle_ai_chat(message):
    ...
    if res and res[0] > 0:
        ...
        conn.close()
        bot.reply_to(message, "❌ نفد رصيد مجاني للذكاء الاصطناعي.")
