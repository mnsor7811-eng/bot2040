...
def fetch_actual_price(srv, srv_code, country_code):
    """جلب السعر الحقيقي للدولة قبل الشراء مباشرة"""
    try:
        url = f"{srv['url']}?api_key={srv['api_key']}&action=getPrices&service={srv_code}&country={country_code}"
        res = requests.get(url, timeout=5).json()
        cost = float(res[str(country_code)][srv_code]['cost'])
        return round(cost * (1 + PROFIT_MARGIN), 2)
    except:
        return 0.30 # سعر افتراضي في حال فشل API الفحص المباشر
...
def countries_keyboard_fast(server_id, service_code):
    ...
    for code, name in POPULAR_COUNTRIES:
        # عرض اسم الدولة فقط مع زر الشراء المباشر
        markup.add(InlineKeyboardButton(f"{name}", callback_data=f"b_{server_id}_{service_code}_{code}"))
...
def callback_handler(call):
    ...
    if call.data == "back_main":
        ...
        # 1. جلب السعر الفعلي فوراً
        price = fetch_actual_price(srv, srv_code, country_code)
        ...
        if "ACCESS_NUMBER" in res:
            ...
            conn.close()
            bot.send_message(chat_id, f"❌ **تنبيه:** لا توجد أرقام متوفرة حالياً لهذه الدولة أو رصيدك في حساب السيرفر غير كافٍ.\nالسعر المستهدف: ${price:.2f}")
