...
def get_grizzly_price(service_code, country_code='0', server_id='grizzly'):
    srv = SERVERS.get(server_id)
    if not srv or srv['api_key'].startswith('ضع_'):
        return 0.50
    try:
        res = requests.get(f"{srv['url']}?api_key={srv['api_key']}&action=getPrices&service={service_code}&country={country_code}", timeout=5).json()
        raw_price = float(res[country_code][service_code]['cost'])
...
def countries_keyboard(server_id, service_code):
    markup = InlineKeyboardMarkup()
    countries = [
        ("🇺🇸 أمريكا", "187"),
        ("🇷🇺 روسيا", "0"),
    ...
    for name, code in countries:
        price = get_grizzly_price(service_code, code, server_id)
        markup.add(InlineKeyboardButton(f"{name} - ${price}", callback_data=f"buy_num_{server_id}_{service_code}_{code}_{price}"))
...
def callback_handler(call):
    ...
    # ---------------- قسم شراء أرقام افتراضية (تعدد السيرفرات) ----------------
        ...
        bot.edit_message_text(text, chat_id, message_id, reply_markup=countries_keyboard(server_id, srv_code))

    elif call.data.startswith("buy_num_"):
        _, _, server_id, srv_code, country_code, price = call.data.split("_")
        price = float(price)
        ...
        if "ACCESS_NUMBER" in res:
            ...
            conn.close()
            bot.send_message(chat_id, "❌ **تنبيه:** لا توجد أرقام متاحة حالياً أو المفتاح غير مفعل لهذا السيرفر. اختر سيرفر أو دولة أخرى.")
