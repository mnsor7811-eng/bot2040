...
# قاموس واسع لترجمة أرقام الدول إلى أسماء وأعلام
COUNTRY_NAMES = {
    "0": "🇷🇺 روسيا", "1": "🇺🇦 أوكرانيا", "2": "🇰🇿 كازاخستان", "3": "🇨🇳 الصين", "4": "🇵🇭 الفلبين",
    "5": "🇲🇲 ميانمار", "6": "🇮🇩 إندونيسيا", "7": "🇲🇾 ماليزيا", "8": "🇰🇭 كينيا", "9": "🇻🇳 فيتنام",
    "10": "🇰🇬 قيرغيزستان", "11": "🇺🇿 أوزبكستان", "12": "🇰🇭 كمبوديا", "13": "🇩🇪 ألمانيا", "14": "🇱🇹 ليتوانيا",
    "15": "🇵🇱 بولندا", "16": "🇬🇧 بريطانيا", "17": "🇲🇩 مدغشقر", "18": "🇨🇭 سويسرا", "19": "🇳🇬 نيجيريا",
    "20": "🇳🇱 هولندا", "21": "🇪🇬 مصر", "22": "🇮🇳 الهند", "23": "🇮🇪 ايرلندا", "24": "🇱🇻 لاتفيا",
    "25": "🇲🇦 المغرب", "26": "🇬hana غانا", "27": "🇪🇸 إسبانيا", "28": "🇧🇪 بلجيكا", "29": "🇵🇰 باكستان",
    "30": "🇦🇱 ألبانيا", "31": "🇸🇪 السويد", "32": "🇷🇴 رومانيا", "33": "🇨🇴 كولومبيا", "34": "🇪🇪 إستونيا",
    "35": "🇦🇿 أذربيجان", "36": "🇨🇦 كندا", "37": "🇲🇱 مالي", "38": "🇪🇹 إثيوبيا", "39": "🇵🇹 البرتغال",
    "40": "🇧🇩 بنغلاديش", "41": "🇹🇷 تركيا", "42": "🇨🇿 التشيك", "43": "🇱🇰 سريلانكا", "44": "🇵🇪 البيرو",
    "46": "🇹🇭 تايلاند", "47": "🇸🇦 السعودية", "48": "🇮🇶 العراق", "49": "🇿🇦 جنوب إفريقيا",
    "50": "🇨🇮 ساحل العاج", "51": "🇦🇷 الأرجنتين", "52": "🇯🇴 الأردن", "53": "🇧🇾 بيلاروسيا", "54": "🇳🇵 نيبال",
    "55": "🇫🇮 فنلندا", "56": "🇭🇺 المجر", "57": "🇲🇽 المكسيك", "58": "🇩🇿 الجزائر", "59": "🇦🇴 أنغولا",
    "73": "🇧🇷 البرازيل", "78": "🇫🇷 فرنسا", "86": "🇮🇹 إيطاليا", "95": "🇸🇩 السودان", "101": "🇸🇾 سوريا",
    "114": "🇦🇪 الإمارات", "117": "🇾🇪 اليمن", "154": "🇸🇳 السنغال", "155": "🇹🇳 تونس", "156": "🇿🇲 زامبيا",
    "157": "🇿🇼 زيمبابوي", "158": "🇲🇴 ماكاو", "159": "🇲🇼 مالاوي", "160": "🇲🇻 جزر المالديف", "161": "🇲🇹 مالتـا",
    "162": "🇲🇷 موريتانيا", "163": "🇲🇺 موريشيوس", "164": "🇲🇳 منغوليا", "165": "🇲🇪 الجبل الأسود", "166": "🇲🇿 موزمبيق",
    "167": "🇳🇦 ناميبيا", "168": "🇳🇨 كاليدونيا الجديدة", "169": "🇳🇮 نيكاغوارا", "170": "🇳🇪 النيجر", "171": "🇴🇲 عمان",
    "172": "🇵🇦 بنما", "173": "🇵🇬 بابوا غينيا", "174": "🇵🇾 باراغواي", "175": "🇵🇷 بورتوريكو", "176": "🇶🇦 قطر", "187": "🇺🇸 أمريكا"
}
...
# ==================== 4. لوحات الأزرار وعرض 20 دولة مع الصفحات ====================
...
def countries_keyboard_fast(server_id, service_code, page=0):
    ...
    for code, price in current_items:
        c_name = COUNTRY_NAMES.get(str(code), f"🌐 دولة ({code})")
    ...
    # رصف الأزرار في عمودين (زرين لكل صف)
    for i in range(0, len(buttons), 2):
        markup.row(*buttons[i:i+2])

    # إضافة أزرار التنقل (السابق / رقم الصفحة / التالي)
    nav_buttons = []
    if page > 0:
...
def callback_handler(call):
    ...
    if call.data == "back_main":
        ...
        if pay_info:
            ...
            bot.edit_message_text(msg, chat_id, message_id, reply_markup=back_markup)
    ...
    # معالجة التنقل بين صفحات الدول (التالي والسابق)
        ...
        if "ACCESS_NUMBER" in res:
            ...
            bot.send_message(chat_id, msg, reply_markup=active_number_keyboard(tz_id, server_id))
        else:
            conn.close()
...
def handle_user_steps(message):
    ...
    if step_data['step'] == 'TRANSFER_TARGET':
        ...
        if amount < MIN_TRANSFER_AMOUNT:
            bot.send_message(message.chat.id, f"❌ أذن مبلغ للتحويل هو ${MIN_TRANSFER_AMOUNT:.2f}")
            return
...
# الرد الذكي من Gemini AI للرسائل النصية العادية
