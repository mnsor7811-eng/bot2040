...
def clean_phone_number(raw_phone, country_code=None):
    ...
    if country_code is not None:
        ...
        if not dial and len(c_str) >= 4:
            clean_c = c_str.replace("100", "").replace("10", "")
            dial = COUNTRY_DIAL_CODES.get(clean_c)
...
def sync_servers_from_db():
    """مزامنة السيرفرات المضافة من لوحة الأدمن تلقائياً مع قائمة السيرفرات في الذاكرة"""
    try:
        conn = get_db()
        cursor = conn.cursor()
        ...
        for row in cursor.fetchall():
            ...
            if p_id not in SERVERS:
                SERVERS[p_id] = {'name': p_name, 'api_key': p_key, 'url': p_url}
            else:
                if p_key and "ضع_مفتاح" not in p_key:
                    SERVERS[p_id]['api_key'] = p_key
                if p_url:
                    SERVERS[p_id]['url'] = p_url
        conn.close()
    except Exception as e:
        pass
