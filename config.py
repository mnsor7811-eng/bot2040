...
COUNTRY_MAP = {
    ...
    "52": {"name": "تايلاند", "flag": "🇹🇭"}, "53": {"name": "السعودية", "flag": "🇸🇦"},
    "54": {"name": "المكسيك", "flag": "🇲🇽"}, "55": {"name": "تايوان", "flag": "🇹🇼"},
    "56": {"name": "إسبانيا", "flag": "🇪🇸"}, "57": {"name": "الجزائر", "flag": "🇩🇿"},
    "58": {"name": "سلوفينيا", "flag": "🇸🇮"}, "59": {"name": "بنغلاديش", "flag": "🇧🇩"},
    "60": {"name": "البرتغال", "flag": "🇵🇹"}, "61": {"name": "البرازيل", "flag": "🇧🇷"},
    "62": {"name": "تركيا", "flag": "🇹🇷"}, "63": {"name": "تونس", "flag": "🇹🇳"},
    "64": {"name": "أوكرانيا", "flag": "🇺🇦"}, "65": {"name": "السودان", "flag": "🇸🇩"},
    "66": {"name": "أستراليا", "flag": "🇦🇺"}, "67": {"name": "فنلندا", "flag": "🇫🇮"},
    "68": {"name": "فرنسا", "flag": "🇫🇷"}, "69": {"name": "بلجيكا", "flag": "🇧🇪"},
    "70": {"name": "سنغافورة", "flag": "🇸🇬"}, "71": {"name": "بلغاريا", "flag": "🇧🇬"},
    "72": {"name": "المجر", "flag": "🇭🇺"}, "73": {"name": "مولدوفا", "flag": "🇲🇩"},
    "74": {"name": "إيطاليا", "flag": "🇮🇹"}, "75": {"name": "باراغواي", "flag": "🇵🇾"},
    "76": {"name": "هندوراس", "flag": "🇭🇳"}, "77": {"name": "جورجيا", "flag": "🇬🇪"},
    "78": {"name": "اليونان", "flag": "🇬🇷"}, "86": {"name": "إيطاليا", "flag": "🇮🇹"},
...
def buy_ready_account_api(server_id, country_code):
    ...
    if str(server_id) == '3':
        return {
            'ok': False,
            'error': '⚠️ تنبيه المزود: مزود TG-Lion لا يدعم الحسابات القديمة (المعتقة) عبر الـ API الفوري، وإنما يدعم الأرقام الجديدة فقط.\nيرجى استخدام [السيرفر 1] أو [السيرفر 2] للشراء الفوري التلقائي 100%، أو التواصل مع الإدارة لتسليم الحساب القديم يدوياً.'
        }
    ...
    try:
        if p_type == 'tglion':
            ...
            if res and res.get('status') == 'ok' and res.get('Number'):
                ...
                raw_err = (res.get('message') if res else None) or (res.get('error') if res else None) or 'خطأ في المزود'
                if 'balance' in str(raw_err).lower() or 'رصيد' in str(raw_err):
                    err_msg = '⚠️ رصيد حساب المزود (TG-Lion) غير كافٍ لإتمام عملية الشراء، يرجى شحن حساب المزود أولاً.'
                elif 'stock' in str(raw_err).lower() or 'no number' in str(raw_err).lower():
                    err_msg = '⚠️ نفد مخزون هذه الدولة حالياً لدى المزود، يرجى اختيار دولة أخرى أو المحاولة لاحقاً.'
                elif 'banned' in str(raw_err).lower():
                    err_msg = '⚠️ الرقم محظور أو غير صالح لدى المزود.'
                else:
            ...
            if res and res.get('ok') and isinstance(res.get('result'), dict):
                ...
                raw_err = (res.get('error') if res else None) or (res.get('msg') if res else None) or 'خطأ في المزود'
                if 'balance' in str(raw_err).lower() or 'رصيد' in str(raw_err):
                    err_msg = '⚠️ رصيد حساب المزود (Spider) غير كافٍ لإتمام العملية، يرجى شحن حساب المزود.'
                elif 'stock' in str(raw_err).lower() or 'available' in str(raw_err).lower():
                    err_msg = '⚠️ نفد مخزون هذه الدولة لدى المزود، يرجى اختيار دولة أخرى.'
                else:
                    err_msg = f"⚠️ رد المزود: {raw_err}"
                return {'ok': False, 'error': err_msg}
    except Exception as e:
