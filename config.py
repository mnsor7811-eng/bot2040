...
def sync_servers_from_db():
    ...
    try:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("SELECT provider_id, name, api_key, url FROM providers WHERE category = 'numbers' AND is_active = 1")
        for row in cursor.fetchall():
            p_id, p_name, p_key, p_url = row
            # توحيد المعرفات لمنع أي تكرار
            target_id = p_id
