...
# ==================== الإعدادات الأساسية ====================
TOKEN = os.getenv('TOKEN', '8927305428:AAFlmUQsOQr5e5MpHqfx3IAPLGmtk5wuzv4')
        ...
        if server_id in ['grizzly', 'tigersms']:
            ...
            for c_code, services in res.items():
                if isinstance(services, dict) and service_code in services:
                    ...
                    count = int(services[size := service_code].get('count', 0) if hasattr(services[service_code], 'get') else services[service_code].get('count', 0))
                    # الحفاظ على الكود الأصلي تماماً:
