...
def dynamic_smm_keyboard(services_list, category_code, page=0, smm_server_id="2"):
    ...
    for srv in current_items:
        ...
        markup.add(InlineKeyboardButton(display_text, callback_data=f"smmbuy_2_{srv_id}"))
...
def smm_service_detail_keyboard(srv_id, smm_server_id="2"):
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("❇️ : طلب الخدمة", callback_data=f"smm_order_2_{srv_id}"))
    ...
    markup.add(InlineKeyboardButton("🔄 رجوع", callback_data="smm_servers_menu"))
    return markup
...
def smm_detail_grid_keyboard(service_id, price, speed, quality, guarantee, min_q, max_q, smm_server_id="2"):
    ...
    markup.add(InlineKeyboardButton("❇️ : طلب الخدمة", callback_data=f"smm_order_2_{service_id}"))
...
def smm_confirm_keyboard(srv_id, quantity, total_price, smm_server_id="2"):
    markup = InlineKeyboardMarkup()
    markup.row(
        InlineKeyboardButton("✅ تأكيد الطلب", callback_data=f"smm_confirm_2_{srv_id}_{quantity}_{total_price}"),
        InlineKeyboardButton("❌ إلغاء", callback_data="smm_servers_menu")
    )
    return markup
