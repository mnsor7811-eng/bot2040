...
def main_keyboard(user_id):
    ...
    if is_section_enabled('store'):
        row_1.append(InlineKeyboardButton("🛍️ المتجر والخدمات الرقمية", callback_data="store_menu"))
    if is_section_enabled('ai'):
        row_1.append(InlineKeyboardButton("🤖 اشتراكات AI", callback_data="ai_landing"))
    if row_1:
        markup.row(*row_1)

    # صف الأرقام الوهمية وعروض تيليجرام
    row_2 = []
    if is_section_enabled('numbers'):
        row_2.append(InlineKeyboardButton("📞 شراء أرقام وهمية", callback_data="buy_number"))
        row_2.append(InlineKeyboardButton("🔵 عروض Telegram", callback_data="fast_buy_tg_servers"))
    if row_2:
        markup.row(*row_2)

    # الحسابات الجاهزة وعروض واتساب
    row_3 = []
    if is_section_enabled('ready'):
        row_3.append(InlineKeyboardButton("💯 حسابات تيليجرام جاهزة", callback_data="ready_accounts_menu"))
    row_3.append(InlineKeyboardButton("🟢 عروض WhatsApp", callback_data="fast_buy_wa"))
    markup.row(*row_3)

    # شحن الرصيد والرشق
    row_4 = []
    if is_section_enabled('recharge'):
        row_4.append(InlineKeyboardButton("🎳 شحن الرصيد / الاشتراكات", callback_data="recharge_menu"))
    if is_section_enabled('smm'):
        row_4.append(InlineKeyboardButton("🚀 خدمات الرشق والألعاب", callback_data="smm_main"))
    if row_4:
        markup.row(*row_4)

    # اربح رصيد وتحويل الرصيد
    row_5 = []
    if is_section_enabled('free'):
        row_5.append(InlineKeyboardButton("💎 اربح رصيد مجانا", callback_data="free_ruble"))
    if is_section_enabled('transfer'):
        row_5.append(InlineKeyboardButton("🔄 تحويل الرصيد", callback_data="transfer"))
    if row_5:
        markup.row(*row_5)

    # الدعم وحسابي
    markup.row(InlineKeyboardButton("🎧 الدعم الفني", callback_data="support"), InlineKeyboardButton("👤 حسابي", callback_data="my_account"))
    markup.row(InlineKeyboardButton("✔ إحصائيات الشراء الناجح", callback_data="purchase_stats"), InlineKeyboardButton("⚙️ الإعدادات والمزيد", callback_data="more_settings_menu"))
...
def binance_txid_fail_keyboard():
    markup = InlineKeyboardMarkup()
    markup.row(
        InlineKeyboardButton("🔄 إعادة إدخال TXID", callback_data="enter_txid"),
        InlineKeyboardButton("🔙 رجوع", callback_data="recharge_menu")
    )
    return markup
