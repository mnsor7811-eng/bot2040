...
def admin_smm_custom_keyboard():
    markup = InlineKeyboardMarkup()
    markup.row(InlineKeyboardButton("➕ إضافة خدمة رشق جديدة يدوياً", callback_data="adm_smm_add"))
    markup.row(InlineKeyboardButton("📋 عرض وتعديل الخدمات المضافة", callback_data="adm_smm_list_0"))
    markup.row(InlineKeyboardButton("🔙 رجوع لوحة الإدارة", callback_data="admin_panel"))
    return markup

def admin_smm_services_list_keyboard(page=0):
    markup = InlineKeyboardMarkup()
    ...
    for srv in current_items:
        ...
        srv_name = srv['name']
        srv_price = srv['rate']
        srv_server = "سيرفر 2 (Tiger)" if srv['server_id'] == 'tiger' else f"سيرفر {srv['server_id']}"
        markup.row(
            InlineKeyboardButton(f"{srv_name[:25]} | {srv_price}$", callback_data=f"adm_smm_view_{srv_db_id}"),
            InlineKeyboardButton("🗑️", callback_data=f"adm_smm_del_{srv_db_id}")
        )

    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️ السابق", callback_data=f"adm_smm_list_{page-1}"))
    nav.append(InlineKeyboardButton(f"📄 {page+1}/{total_pages}", callback_data="ignore"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton("التالي ➡️", callback_data=f"adm_smm_list_{page+1}"))
    if nav:
        markup.row(*nav)

    markup.row(InlineKeyboardButton("➕ إضافة خدمة جديدة", callback_data="adm_smm_add"))
...
def smm_servers_menu_keyboard():
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("🚀 سيرفر الرشق 1 (SMM Xtar - تلقائي)", callback_data="smm_srv_2"),
        InlineKeyboardButton("🐅 سيرفر الرشق 2 (SMM Tigers - معتمد وسريع)", callback_data="smm_srv_tiger"),
...
def boost_keyboard(smm_server_id="2"):
    ...
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("📱 - رشق تيليجرام . telegram", callback_data=f"smmc_{s_id}_telegram"),
        InlineKeyboardButton("📸 - رشق انستا . instagram", callback_data=f"smmc_{s_id}_instagram"),
        InlineKeyboardButton("▶️ - رشق يوتيوب . youtube", callback_data=f"smmc_{s_id}_youtube"),
        InlineKeyboardButton("🐦 - رشق تويتر . twitter", callback_data=f"smmc_{s_id}_twitter"),
        InlineKeyboardButton("📘 - رشق فيسبوك . facebook", callback_data=f"smmc_{s_id}_facebook"),
        InlineKeyboardButton("🎵 - رشق تيك توك . tiktok", callback_data=f"smmc_{s_id}_tiktok"),
        InlineKeyboardButton("👤 - رشق ثريدز . threads", callback_data=f"smmc_{s_id}_threads"),
        InlineKeyboardButton("🟢 - واتس اب . whatsapp", callback_data=f"smmc_{s_id}_whatsapp"),
        InlineKeyboardButton("➕ - خدمات اخرى . other services", callback_data=f"smmc_{s_id}_others"),
        InlineKeyboardButton("🔙 رجوع للسيرفرات", callback_data="smm_servers_menu")
    )
    return markup
