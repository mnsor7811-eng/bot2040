...
def recharge_keyboard():
    ...
    markup.add(InlineKeyboardButton("📱 محفظة جيب (تلقائي)", callback_data="pay_jeeb_auto"))
...
def jeeb_payment_action_keyboard(account_number):
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("✅ تم الدفع (أدخل رقم المعاملة)", callback_data="jeeb_enter_txid"),
        InlineKeyboardButton(f"📋 نسخ رقم المحفظة ({account_number})", callback_data=f"copy_jeeb_acc_{account_number}"),
        InlineKeyboardButton("🔙 رجوع", callback_data="recharge_menu")
    )
    return markup

def jeeb_cancel_keyboard():
    markup = InlineKeyboardMarkup()
