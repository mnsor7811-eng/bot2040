...
def recharge_keyboard():
    """قائمة وسائل الدفع المعتمدة رسمياً"""
    markup = InlineKeyboardMarkup()
...
def servers_keyboard(user_id=None):
    """عرض السيرفرات مع إخفاء أسماء المزودين الحقيقية وتسميتها بـ سيرفر الأرقام 1، 2..."""
    markup = InlineKeyboardMarkup()
    is_admin = (str(user_id) == str(ADMIN_ID)) if user_id else False
    
    for idx, (srv_id, srv_info) in enumerate(SERVERS.items(), start=1):
        # إظهار الاسم الحقيقي للأدمن فقط، واسم مموه برقم للمستخدم
...
def smm_servers_keyboard():
    """عرض قائمة سيرفرات الرشق لاختيار السيرفر المناسب أولاً"""
    markup = InlineKeyboardMarkup()
...
def boost_keyboard(smm_server_id="1"):
    """عرض منصات الرشق المرتبطة بالسيرفر المختار"""
...
def dynamic_smm_keyboard(services_list, category_code, page=0, smm_server_id="1"):
    markup = InlineKeyboardMarkup(row_width=1)
...
def smm_service_detail_keyboard(srv_id, smm_server_id="1"):
    markup = InlineKeyboardMarkup()
...
def smm_detail_grid_keyboard(service_id, price, speed, quality, guarantee, min_q, max_q, smm_server_id="1"):
    markup = InlineKeyboardMarkup()
...
def smm_confirm_keyboard(srv_id, quantity, total_price, smm_server_id="1"):
    markup = InlineKeyboardMarkup()
    markup.row(
