...
def get_store_keyboard():
    ...
    keyboard.add(
        telebot.types.InlineKeyboardButton("🎁 بطاقات الهدايا", callback_data="cat_giftcards"),
        telebot.types.InlineKeyboardButton("🎮 شحن الألعاب", callback_data="cat_gaming"),
        telebot.types.InlineKeyboardButton("🌐 خدمات الإنترنت والبرمجيات", callback_data="cat_services"),
        telebot.types.InlineKeyboardButton("🔙 القائمة الرئيسية", callback_data="back_main")
    )
    return keyboard
...
def get_back_to_store_keyboard():
    keyboard = telebot.types.InlineKeyboardMarkup(row_width=1)
    keyboard.add(
        telebot.types.InlineKeyboardButton("🔙 عودة للمتجر", callback_data="store_menu")
    )
    return keyboard

def main_keyboard(user_id):
    markup = InlineKeyboardMarkup()
    markup.row(InlineKeyboardButton("🛍️ المتجر والخدمات الرقمية", callback_data="store_menu"))
