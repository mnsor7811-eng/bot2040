...
from config import (
    ...
    fetch_server_prices, get_clean_country_info
)
...
# ==================== لوحة الإدارة الكبرى الموسعة ====================
def admin_panel_keyboard():
    markup = InlineKeyboardMarkup()
    # الصف 1: شحن الرصيد السريع والشحن الذاتي للأدمن
    markup.row(
        InlineKeyboardButton("⚡ شحن ذاتي لي ($50)", callback_data="admin_self_charge_50"),
        InlineKeyboardButton("💎 شحن ذاتي لي ($10)", callback_data="admin_self_charge_10")
    )
    markup.row(
    ...
    # الصف 5: الطلبات الأخيرة وإعدادات البوت
    markup.row(
    ...
    markup.row(
        InlineKeyboardButton("📈 تعديل نسبة الربح", callback_data="admin_set_profit"),
...
# ==================== قائمة الإعدادات والمزيد (مطابقة للصورة 1) ====================
def more_settings_keyboard():
    markup = InlineKeyboardMarkup()
    markup.row(
        InlineKeyboardButton("📢 القناة الرسمية", url=CHANNEL_OFFICIAL_URL),
        InlineKeyboardButton("🛍️ قناة الطلبات", url=CHANNEL_ORDERS_URL)
    )
    markup.row(
...
# ==================== قائمة سيرفرات الحسابات الجاهزة ====================
def ready_accounts_keyboard():
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("🦁 السيرفر 1 (TG-Lion API)", callback_data="ready_server_1"))
    markup.add(InlineKeyboardButton("🕷️ السيرفر 2 (Spider TG API)", callback_data="ready_server_2"))
    markup.add(InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="back_main"))
    return markup

# ==================== سيرفرات عروض تيليجرام المباشرة ====================
def tg_servers_keyboard():
    markup = InlineKeyboardMarkup()
    for idx, (srv_id, srv_info) in enumerate(SERVERS.items(), start=1):
        markup.add(InlineKeyboardButton(f"🔵 سيرفر تليجرام {idx} ({srv_info['name']})", callback_data=f"srv_app_{srv_id}_tg"))
    markup.add(InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="back_main"))
    return markup
