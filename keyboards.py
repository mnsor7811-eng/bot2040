...
from config import (
    ADMIN_ID, SERVERS, SMM_SERVERS, CHANNEL_OFFICIAL_URL, CHANNEL_ORDERS_URL,
    CHANNEL_TUTORIALS_URL, CHANNEL_EXPLAINS_URL,
    ...
    get_custom_smm_services, POPULAR_SERVICES
)
...
def translate_text(text: str) -> str:
    translations = {
        ...
        "whatsapp": "واتساب", "threads": "ثريدز", "snapchat": "سناب شات",
        ...
        "refill": "ضمان تعويض", "guarantee": "ضمان", "target": "مستهدف",
        "arab": "عرب", "arabic": "عربي", "organic": "عضوي", "high quality": "جودة عالية",
        "hq": "جودة عالية", "best": "الأفضل", "speed": "سرعة فائقة",
        "channel": "قناة", "group": "مجموعة", "post": "منشور", "video": "فيديو",
        "reactions": "تفاعلات", "reaction": "تفاعل", "votes": "تصويتات", "poll": "استطلاع",
        "female": "إناث", "male": "ذكور", "gulf": "خليجي", "saudi": "سعودي", "iraq": "عراقي",
        "egypt": "مصري", "premium": "بريميوم", "star": "نجوم", "boost": "بوستات دعم"
    }
    translated = str(text)
...
def main_keyboard(user_id):
    ...
    # 1. الأزرار الصغيرة المتقابلة: عروض واتساب يقابله عروض تيليجرام (بدون إيموجي لتكون مدمجة)
    row_offers = []
    ...
    # 2. شراء أرقام وهمية يقابله اشتراكات AI
    row_numbers_ai = []
    ...
    if is_section_enabled('ai'):
        row_numbers_ai.append(InlineKeyboardButton("🤖 اشتراكات AI", callback_data="ai_landing"))
    if row_numbers_ai:
    ...
    if is_section_enabled('store'):
        row_store_smm.append(InlineKeyboardButton("🛍️ الخدمات الرقمية", callback_data="store_menu"))
    if is_section_enabled('smm'):
        row_store_smm.append(InlineKeyboardButton("🚀 الرشق والألعاب", callback_data="smm_main"))
    if row_store_smm:
...
def admin_panel_keyboard():
    ...
    markup.row(
        InlineKeyboardButton("⚡ إدارة خدمات الرشق المخصصة", callback_data="adm_smm_custom"),
    ...
    markup.row(
        InlineKeyboardButton("💎 إعدادات (اربح رصيد مجانا)", callback_data="adm_referrals"),
    ...
    markup.row(
        InlineKeyboardButton("🛡️ التحقق الأمني (Anti-Bot)", callback_data="adm_captcha_menu"),
...
def admin_channels_keyboard():
    ...
    markup.row(InlineKeyboardButton("📚 تعديل قناة التعليمات", callback_data="adm_edit_ch_tutorials"))
    markup.row(InlineKeyboardButton("🎬 تعديل قناة الشروحات", callback_data="adm_edit_ch_explains"))
...
# 9. كيبورد إعدادات اربح رصيد مجاناً (نظام الأرباح والشراكة)
def admin_referrals_keyboard():
    ...
    markup.row(InlineKeyboardButton(f"🎁 مكافأة الدعوة الواحدة: ${rew} ✏️", callback_data="adm_edit_ref_reward"))
    markup.row(InlineKeyboardButton(f"📈 عمولة المشتريات: {pct}% ✏️", callback_data="adm_edit_ref_pct"))
    markup.row(InlineKeyboardButton(f"🏧 الحد الأدنى لتحويل الأرباح: ${min_w} ✏️", callback_data="adm_edit_ref_min"))
    markup.row(InlineKeyboardButton(f"حالة قسم الأرباح: {sec_st}", callback_data="adm_tgl_free_sec"))
    markup.row(InlineKeyboardButton("👥 كشف أرباح المستخدمين وأعلى المحيلين 📊", callback_data="adm_view_top_refs"))
    markup.row(InlineKeyboardButton("🔙 رجوع لوحة الإدارة", callback_data="admin_panel"))
    return markup

# كيبورد إعدادات التحقق الأمني (Anti-Bot)
def admin_captcha_keyboard():
    markup = InlineKeyboardMarkup()
    c_st = "✅ مفعل" if get_setting('captcha_enabled', '1') == '1' else "❌ معطل"
    markup.row(InlineKeyboardButton(f"حالة التحقق الأمني: {c_st}", callback_data="adm_tgl_captcha"))
    markup.row(InlineKeyboardButton("🔙 رجوع لوحة الإدارة", callback_data="admin_panel"))
    return markup
...
def more_settings_keyboard():
    ...
    markup = InlineKeyboardMarkup()
    markup.row(
        InlineKeyboardButton("📢 القناة الرسمية", url=ch_off),
        InlineKeyboardButton("🛍️ قناة التفعيلات", url=ch_ord)
    )
    markup.row(
        InlineKeyboardButton("📚 قناة التعليمات", url=ch_tut),
        InlineKeyboardButton("🎬 قناة الشروحات", url=ch_exp)
    )
    markup.row(InlineKeyboardButton("📦 طلباتي", callback_data="my_orders_menu"))
...
# كيبوردات قسم (اربح رصيد مجانا) بنظام الشراكة والأرباح المتطور
def free_balance_keyboard(user_id, bot_uname):
    markup = InlineKeyboardMarkup()
    ref_link = f"https://t.me/{bot_uname}?start={user_id}"
    share_url = f"https://t.me/share/url?url={ref_link}&text=🔥 انضم الآن إلى أقوى بوت لشراء وتفعيل الأرقام الوهمية وخدمات الرشق والحسابات الجاهزة فورياً!"
    markup.row(InlineKeyboardButton("🔗 مشاركة رابط الدعوة", url=share_url))
    markup.row(
        InlineKeyboardButton("📊 إحصائيات فريقي", callback_data="ref_team_stats"),
        InlineKeyboardButton("💸 أرباحي وسحب الرصيد", callback_data="ref_my_earnings")
    )
    markup.row(InlineKeyboardButton("🔙 القائمة الرئيسية", callback_data="back_main"))
    return markup

def free_balance_team_keyboard():
    markup = InlineKeyboardMarkup()
    markup.row(InlineKeyboardButton("🔙 العودة لقسم الأرباح", callback_data="free_ruble"))
    markup.row(InlineKeyboardButton("🏠 القائمة الرئيسية", callback_data="back_main"))
    return markup

def free_balance_earnings_keyboard(can_withdraw=False, earn_amount=0.0):
    markup = InlineKeyboardMarkup()
    min_w = float(get_setting('min_invite_withdraw', '0.5'))
    if can_withdraw and earn_amount > 0:
        markup.row(InlineKeyboardButton(f"🔄 تحويل الأرباح (${earn_amount:.2f}) إلى رصيد حسابي", callback_data="withdraw_ref_earnings"))
    else:
        markup.row(InlineKeyboardButton(f"⏳ تحويل الأرباح إلى رصيدي (الحد الأدنى ${min_w:.2f})", callback_data="withdraw_ref_earnings"))
    markup.row(InlineKeyboardButton("🔙 العودة لقسم الأرباح", callback_data="free_ruble"))
    markup.row(InlineKeyboardButton("🏠 القائمة الرئيسية", callback_data="back_main"))
    return markup
...
def ready_accounts_keyboard():
    ...
    markup.add(InlineKeyboardButton("السيرفر 3", callback_data="ready_server_3"))
    markup.add(InlineKeyboardButton("السيرفر 4 (حسابات قديمة)", callback_data="ready_server_4"))
    markup.add(InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="back_main"))
    return markup
...
def ready_accounts_countries_keyboard(server_id='1', age=None, page=0):
    ...
    back_cb = "ready_server_4" if str(server_id) == "4" and age else "ready_accounts_menu"
...
def no_numbers_keyboard(server_id, service_code='wa', country_code='6', page=0, origin=None):
    markup = InlineKeyboardMarkup()
    markup.row(InlineKeyboardButton("✤ إعادة المحاولة ↺", callback_data=f"b_{server_id}_{service_code}_{country_code}"))
    if origin:
        markup.row(InlineKeyboardButton("✤ عودة ↩", callback_data=f"pg_{server_id}_{service_code}_{page}_{origin}"))
    else:
        markup.row(InlineKeyboardButton("✤ عودة ↩", callback_data=f"pg_{server_id}_{service_code}_{page}"))
    return markup

def completed_number_keyboard(server_id, service_code='wa', country_code='6'):
    markup = InlineKeyboardMarkup()
    markup.row(InlineKeyboardButton("✤ شراء من نفس الدولة ↺", callback_data=f"b_{server_id}_{service_code}_{country_code}"))
    markup.row(InlineKeyboardButton("✤ إنهاء وشراء المزيد ↩", callback_data=f"pg_{server_id}_{service_code}_0"))
    return markup
...
# ==================== كيبوردات لوحة تحكم خدمات الرشق المخصصة (الأدمن) ====================
def admin_smm_custom_keyboard():
    markup = InlineKeyboardMarkup()
    markup.row(InlineKeyboardButton("➕ إضافة خدمة رشق جديدة", callback_data="adm_smm_choose_cat"))
    markup.row(InlineKeyboardButton("📂 إدارة وحذف الخدمات حسب التطبيق", callback_data="adm_smm_cats_menu"))
    markup.row(InlineKeyboardButton("📋 عرض جميع الخدمات المضافة", callback_data="adm_smm_list_all_0"))
    markup.row(
        InlineKeyboardButton("💾 تصدير وحفظ الخدمات (JSON)", callback_data="adm_smm_export_json"),
        InlineKeyboardButton("📥 استيراد خدمات الرشق", callback_data="adm_smm_import_json")
    )
    markup.row(InlineKeyboardButton("🔙 رجوع لوحة الإدارة", callback_data="admin_panel"))
    return markup

def admin_smm_select_app_keyboard(action_type="add"):
    """كيبورد اختيار التطبيق لإضافة خدمة جديدة أو استعراضها"""
    markup = InlineKeyboardMarkup(row_width=2)
    prefix = "adm_smm_setcat_" if action_type == "add" else "adm_smm_list_"
    suffix = "" if action_type == "add" else "_0"
    markup.add(
        InlineKeyboardButton("📱 تيليجرام", callback_data=f"{prefix}telegram{suffix}"),
        InlineKeyboardButton("📸 إنستغرام", callback_data=f"{prefix}instagram{suffix}"),
        InlineKeyboardButton("🎵 تيك توك", callback_data=f"{prefix}tiktok{suffix}"),
        InlineKeyboardButton("🟢 واتساب", callback_data=f"{prefix}whatsapp{suffix}"),
        InlineKeyboardButton("👤 ثريدز", callback_data=f"{prefix}threads{suffix}"),
        InlineKeyboardButton("🐦 تويتر (X)", callback_data=f"{prefix}twitter{suffix}"),
        InlineKeyboardButton("▶️ يوتيوب", callback_data=f"{prefix}youtube{suffix}"),
        InlineKeyboardButton("📘 فيسبوك", callback_data=f"{prefix}facebook{suffix}"),
        InlineKeyboardButton("➕ خدمات أخرى", callback_data=f"{prefix}others{suffix}")
    )
    if action_type != "add":
        markup.add(InlineKeyboardButton("🌐 عرض جميع الخدمات", callback_data="adm_smm_list_all_0"))
    markup.add(InlineKeyboardButton("🔙 رجوع", callback_data="adm_smm_custom"))
    return markup

def admin_smm_categories_view_keyboard():
    """عرض قائمة التطبيقات مع عدد الخدمات المضافة في كل تطبيق"""
    markup = InlineKeyboardMarkup(row_width=2)
    from config import get_custom_smm_categories_counts
    counts = get_custom_smm_categories_counts(server_id='tiger')
    ...
    buttons = []
    for label, code in apps:
        cnt = counts.get(code, 0)
        buttons.append(InlineKeyboardButton(f"{label} ({cnt})", callback_data=f"adm_smm_list_{code}_0"))
    
    markup.add(*buttons)
    markup.add(InlineKeyboardButton("🌐 عرض جميع الخدمات المضافة", callback_data="adm_smm_list_all_0"))
    markup.add(InlineKeyboardButton("🔙 رجوع", callback_data="adm_smm_custom"))
    return markup

def admin_smm_services_list_keyboard(category='all', page=0):
    markup = InlineKeyboardMarkup()
    services = get_custom_smm_services(server_id='tiger', category=category if category != 'all' else None)
    per_page = 8
    ...
    for srv in current_items:
        srv_db_id = srv['db_id']
        srv_name = srv['name_ar'] or srv['name']
        srv_price = srv['rate']
        markup.row(
            InlineKeyboardButton(f"💰 {srv_price}$ | {srv_name[:24]}", callback_data=f"adm_smm_view_{srv_db_id}"),
            InlineKeyboardButton("🗑️ حذف", callback_data=f"adm_smm_del_{srv_db_id}_{category}_{page}")
        )

    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️ السابق", callback_data=f"adm_smm_list_{category}_{page-1}"))
    nav.append(InlineKeyboardButton(f"📄 {page+1}/{total_pages}", callback_data="ignore"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton("التالي ➡️", callback_data=f"adm_smm_list_{category}_{page+1}"))
    if nav:
        markup.row(*nav)

    if category != 'all':
        markup.row(InlineKeyboardButton("➕ إضافة خدمة لهذا التطبيق", callback_data=f"adm_smm_setcat_{category}"))
        markup.row(InlineKeyboardButton("🔙 رجوع للأقسام", callback_data="adm_smm_cats_menu"))
    else:
        markup.row(InlineKeyboardButton("➕ إضافة خدمة جديدة", callback_data="adm_smm_choose_cat"))
        markup.row(InlineKeyboardButton("🔙 رجوع", callback_data="adm_smm_custom"))
    return markup

def admin_smm_speed_keyboard():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("⚡ فورية (Instant)", callback_data="adm_smm_setspd_instant"),
        InlineKeyboardButton("⏱️ 1 - 5 دقائق", callback_data="adm_smm_setspd_5m"),
        InlineKeyboardButton("⏱️ 15 - 30 دقيقة", callback_data="adm_smm_setspd_30m"),
        InlineKeyboardButton("⏳ 1 - 3 ساعات", callback_data="adm_smm_setspd_3h"),
        InlineKeyboardButton("⏳ 6 - 12 ساعة", callback_data="adm_smm_setspd_12h"),
        InlineKeyboardButton("📆 24 - 48 ساعة", callback_data="adm_smm_setspd_24h"),
        InlineKeyboardButton("✍️ كتابة وقت مخصص", callback_data="adm_smm_setspd_custom"),
        InlineKeyboardButton("تخطي الافتراضي ⚡", callback_data="adm_smm_setspd_instant")
    )
    markup.add(InlineKeyboardButton("🔙 إلغاء والرجوع", callback_data="adm_smm_custom"))
    return markup

def admin_smm_guarantee_keyboard():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("بدون ضمان ⚠️", callback_data="adm_smm_setg_none"),
        InlineKeyboardButton("ضمان 30 يوم ♻️", callback_data="adm_smm_setg_30d"),
        InlineKeyboardButton("ضمان 60 يوم ♻️", callback_data="adm_smm_setg_60d"),
        InlineKeyboardButton("ضمان 90 يوم ♻️", callback_data="adm_smm_setg_90d"),
        InlineKeyboardButton("ضمان 365 يوم ♻️", callback_data="adm_smm_setg_365d"),
        InlineKeyboardButton("ضمان مدى الحياة ♾️", callback_data="adm_smm_setg_lifetime"),
        InlineKeyboardButton("تخطي الافتراضي ⚡", callback_data="adm_smm_setg_skip")
    )
    markup.add(InlineKeyboardButton("🔙 إلغاء والرجوع", callback_data="adm_smm_custom"))
    return markup

def smm_servers_menu_keyboard():
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("⚡ سيرفر الرشق 1", callback_data="smm_srv_2"),
        InlineKeyboardButton("🔥 سيرفر الرشق 2 (الأرخص)", callback_data="smm_srv_tiger"),
        InlineKeyboardButton("🔙 الصفحة الرئيسية", callback_data="back_main")
    )
    return markup

def boost_keyboard(smm_server_id="2"):
    s_id = str(smm_server_id)
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("📱 رشق تيليجرام", callback_data=f"smmc_{s_id}_telegram"),
        InlineKeyboardButton("📸 رشق إنستغرام", callback_data=f"smmc_{s_id}_instagram"),
        InlineKeyboardButton("🎵 رشق تيك توك", callback_data=f"smmc_{s_id}_tiktok"),
        InlineKeyboardButton("🟢 رشق واتساب", callback_data=f"smmc_{s_id}_whatsapp"),
        InlineKeyboardButton("👤 رشق ثريدز", callback_data=f"smmc_{s_id}_threads"),
        InlineKeyboardButton("🐦 رشق تويتر (X)", callback_data=f"smmc_{s_id}_twitter"),
        InlineKeyboardButton("▶️ رشق يوتيوب", callback_data=f"smmc_{s_id}_youtube"),
        InlineKeyboardButton("📘 رشق فيسبوك", callback_data=f"smmc_{s_id}_facebook"),
        InlineKeyboardButton("➕ خدمات أخرى", callback_data=f"smmc_{s_id}_others")
    )
    markup.add(InlineKeyboardButton("🔙 رجوع للسيرفرات", callback_data="smm_servers_menu"))
    return markup
...
def dynamic_smm_keyboard(services_list, category_code, page=0, smm_server_id="2"):
    ...
    for srv in current_items:
        ...
        raw_name = str(srv.get('name_ar') or srv.get('name', 'بدون اسم'))
        translated_name = translate_text(raw_name) if not srv.get('is_custom') else raw_name
        try: rate = float(srv.get('rate', 0))
        except: rate = 0.0
        # للخدمات المخصصة، السعر محدد مباشرة من الأدمن، وللخدمات التلقائية نضيف هامش ربح
        if srv.get('is_custom'):
            price_display = rate
        else:
            profit_margin = float(get_setting('profit_margin_smm', '0.10'))
            price_display = round(rate * (1.0 + profit_margin), 4)

        short_name = translated_name[:50] + ("..." if len(translated_name) > 50 else "")
        display_text = f"💰 {price_display:.3f}$ | {short_name}"
        markup.add(InlineKeyboardButton(display_text, callback_data=f"smmbuy_{s_id}_{srv_id}_{category_code}"))
        
    nav_buttons = []
    if page > 0:
        nav_buttons.append(InlineKeyboardButton("⬅️ السابق", callback_data=f"smmp_{s_id}_{category_code}_{page-1}"))
    nav_buttons.append(InlineKeyboardButton(f"📄 {page+1}/{total_pages}", callback_data="ignore"))
    if page < total_pages - 1:
        nav_buttons.append(InlineKeyboardButton("التالي ➡️", callback_data=f"smmp_{s_id}_{category_code}_{page+1}"))
        
    if nav_buttons:
        markup.row(*nav_buttons)
    markup.add(InlineKeyboardButton("🔙 رجوع للأقسام", callback_data=f"smm_srv_{s_id}"))
    return markup
...
def smm_detail_grid_keyboard(service_id, price, speed, quality, guarantee, min_q, max_q, category_code="others", smm_server_id="2"):
    ...
    markup.add(InlineKeyboardButton("✳️ : طلب الخدمة", callback_data=f"smm_order_{s_id}_{service_id}_{category_code}"))
    ...
    markup.add(InlineKeyboardButton("🔄 رجوع", callback_data=f"smmc_{s_id}_{category_code}"))
    return markup

def smm_cancel_link_keyboard(service_id, category_code, smm_server_id="2"):
    s_id = str(smm_server_id)
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("🔄 رجوع", callback_data=f"smmbuy_{s_id}_{service_id}_{category_code}"))
    return markup

def smm_confirm_keyboard(srv_id, quantity, total_price, category_code="others", smm_server_id="2"):
    s_id = str(smm_server_id)
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("☑️ - تأكيد الطلب", callback_data=f"smm_confirm_{s_id}_{srv_id}_{quantity}_{total_price}_{category_code}"))
    markup.add(InlineKeyboardButton("⚠️ - إلغاء الطلب", callback_data=f"smmc_{s_id}_{category_code}"))
    return markup
