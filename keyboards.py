...
def tg_servers_keyboard():
    ...
    for idx, (srv_id, srv_info) in enumerate(SERVERS.items(), start=1):
        name = srv_info.get('name', f"سيرفر {idx}")
        markup.add(InlineKeyboardButton(f"🔵 سيرفر تليجرام {idx} ({name})", callback_data=f"tg_srv_{srv_id}"))
    markup.add(InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="back_main"))
    return markup
...
def wa_servers_keyboard():
    ...
    for idx, (srv_id, srv_info) in enumerate(SERVERS.items(), start=1):
        name = srv_info.get('name', f"سيرفر {idx}")
        markup.add(InlineKeyboardButton(f"🟢 سيرفر واتساب {idx} ({name})", callback_data=f"wa_srv_{srv_id}"))
    markup.add(InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="back_main"))
    return markup
