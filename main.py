...
from keyboards import (
    ...
    smm_service_detail_keyboard, smm_detail_grid_keyboard, dynamic_smm_keyboard, smm_confirm_keyboard
)
...
# ==================== إعدادات API سيرفرات الرشق (SMM Servers) ====================
SMM_SERVERS = {
    'srv1': {
    ...
    'srv2': {
        'url': 'https://smmstone.com/api/v2', # يمكنك تحديث رابط API لسيرفر 2 من هنا
        'key': '01e140e9a06bbce1c5633971a354cf77'
    },
    'srv3': {
        'url': 'https://smmstone.com/api/v2', # يمكنك تحديث رابط API لسيرفر 3 من هنا
        'key': '01e140e9a06bbce1c5633971a354cf77'
    }
}
...
def smm_request(server_id, action, **kwargs):
    srv = SMM_SERVERS.get(server_id, SMM_SERVERS['srv1'])
    payload = {'key': srv['key'], 'action': action}
    payload.update(kwargs)
    try:
        response = requests.post(srv['url'], data=payload, timeout=10)
        return response.json()
    except Exception as e:
        print(f"SMM API Error ({server_id}): {e}")
        return None

def get_cached_smm_services(server_id='srv1'):
...
def filter_smm_services(target_type, server_id='srv1'):
...
def ai_cmd(message):
    bot.send_message(message.chat.id, "🤖 قسم خدمات وااشتراكات الذكاء الاصطناعي\n\nاطرح سؤالك مباشرة في المحادثة وسيجيبك البوت.", reply_markup=back_button())
...
def callback_handler(call):
    ...
    if call.data == "admin_panel":
        ...
        # التعديل رقم 1: معالجة شاملة لمعالج back_main لتجنب حدوث الخطأ وتوقف البوت
        try:
    ...
    # التعديل رقم 2: إضافة خيارات معالجة السيرفرات الثلاثة للرشق secara مستقلة
    elif call.data in ["select_smm_srv_1", "select_smm_srv_2", "select_smm_srv_3"]:
        srv_code = call.data.replace("select_smm_srv_", "srv")
        srv_name = "السيرفر الأول (الرئيسي)" if srv_code == "srv1" else ("السيرفر الثاني" if srv_code == "srv2" else "السيرفر الثالث")
        bot.answer_callback_query(call.id, f"تم اختيار {srv_name}")
        try:
            bot.edit_message_text(f"🐙 **{srv_name}**\n\nتوفر خدمات متابعين وإعجابات ومشاهدات بأسعار مناسبة\n\n🧛‍♂️ الرجاء إختيار الخدمة:", chat_id, message_id, parse_mode="Markdown", reply_markup=boost_keyboard(srv_code))
        except Exception:
            bot.send_message(chat_id, f"🐙 **{srv_name}**\n\nتوفر خدمات متابعين وإعجابات ومشاهدات بأسعار مناسبة\n\n🧛‍♂️ الرجاء إختيار الخدمة:", parse_mode="Markdown", reply_markup=boost_keyboard(srv_code))
        ...
        try: bot.edit_message_text("🐙 توفر خدمات متابعين وإعجابات ومشاهدات بأسعار مناسبة\n\n🧛‍♂️ الرجاء إختيار الخدمة:", chat_id, message_id, reply_markup=boost_keyboard('srv1'))
        except Exception: bot.send_message(chat_id, "🐙 توفر خدمات متابعين وإعجابات ومشاهدات بأسعار مناسبة\n\n🧛‍♂️ الرجاء إختيار الخدمة:", reply_markup=boost_keyboard('srv1'))

    elif call.data.startswith("smmc_"):
        bot.answer_callback_query(call.id, "جاري جلب الخدمات وتحديث الأسعار...")
        parts = call.data.split("_")
        
        # معالجة سيرفر الرشق والمجموعة
        if len(parts) >= 3 and parts[1] in SMM_SERVERS:
            server_id = parts[1]
        ...
        markup = dynamic_smm_keyboard(filtered_services, category_code, page=0, server_id=server_id)
        try:
            ...
            bot.send_message(chat_id, "✅ : جميع الخدمات المتوفرة في هذا القسم 👇\n☑️ : يرجى اختيار الخدمة المناسبة لك 👇\n⚠️ ملاحظة يتم عرض الخدمات كالتالي: اسم الخدمة ▻ السعر لكل 1000", reply_markup=markup)

    elif call.data.startswith("smmp_"):
        parts = call.data.split("_")
        if len(parts) >= 4 and parts[1] in SMM_SERVERS:
            server_id = parts[1]
            category_code = parts[2]
            page = int(parts[3])
        else:
        ...
        filtered_services = filter_smm_services(category_code, server_id)
        markup = dynamic_smm_keyboard(filtered_services, category_code, page=page, server_id=server_id)
        ...
        if call.data.startswith("smmbuy_"):
            ...
            name = selected_srv.get('name', 'خدمة غير محددة')
            min_q = selected_srv.get('min', '1')
            max_q = selected_srv.get('max', '1000000')
            rate = float(selected_srv.get('rate', 0))
            price_profit = round(rate * 1.10, 3)
            
            msg_text = (f"▫️ اسم الخدمة : {name}\n"
                        f"▫️ الخدمة : {selected_srv.get('category', 'عام')}\n"
                        f"▫️ وصف الخدمة :\n"
                        f"⚡ السرعة : عالية وشبه فورية\n"
                        f"♻️ الضمان : تعويض تلقائي عند النقص\n"
                        f"📌 يرجى إدخال الرابط الصحيح بالحقل المخصص.")
            ...
            service_id = parts[1] if len(parts) > 1 else call.data
            USER_STEPS[user_id] = {'step': 'WAITING_LINK', 'service_id': service_id, 'server_id': 'srv1'}
        ...
        parts = call.data.split("_")
        if len(parts) >= 4 and parts[2] in SMM_SERVERS:
            server_id = parts[2]
            service_id = parts[3]
        else:
            server_id = 'srv1'
            service_id = parts[2] if len(parts) > 2 else parts[1]

        USER_STEPS[user_id] = {'step': 'WAITING_LINK', 'service_id': service_id, 'server_id': server_id}
        bot.send_message(chat_id, "🔗 أرسل الرابط أو المعرف (ID) المطلوب للخدمة الآن:", parse_mode="Markdown")

    elif call.data.startswith("smm_confirm_"):
        parts = call.data.split("_")
        if len(parts) >= 6 and parts[2] in SMM_SERVERS:
            server_id = parts[2]
            service_id = parts[3]
            qty = int(parts[4])
...
def handle_user_steps(message):
    ...
    if step == "waiting_binance_amount":
        # التعديل رقم 3: معالجة أخطاء التنسيق (ValueError) لمنع توقف البوت على Railway
        try:
    ...
    # --- معالجة عمليات تحويل الرصيد ---
        ...
        try:
            ...
            try:
                bot.send_message(target_id, f"🎉 وصلك تحويل رصيد بقيمة ${amount:.2f} من مستخدم آخر!")
            except:
                pass
            return
        except (ValueError, TypeError):
    ...
    if str(user_id) == str(ADMIN_ID):
        if step == 'ADMIN_BROADCAST':
            ...
            try:
                ...
                t_id, amount = int(parts[0]), float(parts[1])
                conn = get_db()
                cursor = conn.cursor()
                try:
                    ...
                    conn.close()
                del USER_STEPS[user_id]
                bot.send_message(message.chat.id, f"✅ تم إضافة ${amount:.2f} للحساب `{t_id}` بنجاح.", parse_mode="Markdown", reply_markup=admin_back_button())
            except Exception:
                bot.send_message(message.chat.id, "❌ الصيغة خاطئة. أرسل بالشكل: `ID المبلغ`", parse_mode="Markdown")
            return
            ...
            try:
                ...
                try:
                    ...
                    conn.close()
                del USER_STEPS[user_id]
                bot.send_message(message.chat.id, f"✅ تم خصم ${amount:.2f} من الحساب `{t_id}` بنجاح.", parse_mode="Markdown", reply_markup=admin_back_button())
            except Exception:
                bot.send_message(message.chat.id, "❌ الصيغة خاطئة. أرسل بالشكل: `ID المبلغ`", parse_mode="Markdown")
            return
            ...
            try:
                parts = message.text.strip().split()
                t_id, count = int(parts[0]), int(parts[1])
                conn = get_db()
                cursor = conn.cursor()
                try:
                    cursor.execute("UPDATE users SET ai_balance = ai_balance + ? WHERE user_id = ?", (count, t_id))
                    conn.commit()
                finally:
                    conn.close()
                del USER_STEPS[user_id]
                bot.send_message(message.chat.id, f"✅ تم إضافة {count} أسئلة AI للحساب `{t_id}` بنجاح.", parse_mode="Markdown", reply_markup=admin_back_button())
            except Exception:
                bot.send_message(message.chat.id, "❌ الصيغة خاطئة. أرسل بالشكل: `ID العدد`", parse_mode="Markdown")
            return
            ...
            try:
                ...
                try:
                    ...
                    if res:
                        new_status = 0 if res[0] == 1 else 1
                        cursor.execute("UPDATE users SET is_banned = ? WHERE user_id = ?", (new_status, t_id))
                        conn.commit()
                        bot.send_message(message.chat.id, "🚫 تم حظر المستخدم." if new_status == 1 else "🟢 تم فك حظر المستخدم.", reply_markup=admin_back_button())
                    else:
        ...
        elif step in ['ADMIN_ADD_BALANCE_DIRECT', 'ADMIN_DEDUCT_BALANCE_DIRECT', 'ADMIN_ADD_AI_DIRECT']:
            try:
                target_id = step_data['target_id']
                ...
                try:
                    if step == 'ADMIN_ADD_BALANCE_DIRECT':
                        cursor.execute("UPDATE users SET balance = balance + ? WHERE user_id = ?", (val, target_id))
                    elif step == 'ADMIN_DEDUCT_BALANCE_DIRECT':
                        cursor.execute("UPDATE users SET balance = balance - ? WHERE user_id = ?", (val, target_id))
                    elif step == 'ADMIN_ADD_AI_DIRECT':
                        cursor.execute("UPDATE users SET ai_balance = ai_balance + ? WHERE user_id = ?", (int(val), target_id))
                    conn.commit()
                finally:
                    conn.close()
                del USER_STEPS[user_id]
                ...
                bot.send_message(message.chat.id, "❌ يرجى أدخال قيمة عددية صحيحة.", reply_markup=admin_back_button())
            return
    ...
    if step == 'WAITING_LINK':
        ...
        server_id = step_data.get('server_id', 'srv1')
        link = step_data.get('link')
        
        if str(service_id).startswith("game_"):
            total_cost = round((1.00 / 1000) * qty, 2)
            markup = smm_confirm_keyboard(service_id, qty, total_cost, server_id=server_id)
            bot.send_message(
                message.chat.id,
        ...
        services_data = get_cached_smm_services(server_id)
        service_rate = 0.0
        min_q = 1
        max_q = 1000000
        ...
        markup = smm_confirm_keyboard(service_id, qty, total_cost_with_profit, server_id=server_id)
        bot.send_message(
            message.chat.id,
            f"📋 **تأكيد تفاصيل الطلب:**\n\n"
            f"🔗 الرابط: `{link}`\n"
