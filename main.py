...
def init_db():
    ...
    ''')
    # جدول لطلبات الشحن المعلقة (لتتبع الدفع التلقائي عبر جيب)
    cursor.execute('''
...
def callback_handler(call):
    ...
    # ==================== معالجة أزرار شحن الرصيد وبانكس وجيب (Jeep) التلقائي ====================
    elif call.data == "deposit_jeep" or call.data == "pay_jeep":
        ...
        msg = bot.send_message(
            chat_id,
            "💳 **شحن رصيد عبر تطبيق جيب (Jeep) - تلقائي**\n\n"
            "⚠️ تنبيه مهم ‼️\n"
            "• هذه خدمة دفع تلقائي عبر محفظة جيب.\n"
            "• سيتم إضافة الرصيد إلى حسابك فور وصول الحوالة رقمياً.\n\n"
            "💵 أرسل المبلغ الذي تريد شحنه (بالريال أو العملة المعتمدة):\n"
            "مثال: 1000",
            reply_markup=markup
        )
        USER_STEPS[user_id] = {"step": "waiting_jeep_amount"}
        bot.register_next_step_handler(msg, process_jeep_amount)
        ...
        # جيب اصبحت تلقائية (تم معالجتها في الأعلى)، أما البقية فتبقى يدوية كما طلبت
        if pay_key == "jeep":
            return
...
# ==================== دوال خطوات الدفع والشحن ====================
def process_jeep_amount(message):
    ...
    try:
        ...
        # تسجيل طلب معلق في القاعدة لمطابقته عند وصول الإشعار
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO pending_deposits (user_id, amount, status) VALUES (?, ?, 'PENDING')", (user_id, amount))
        conn.commit()
        conn.close()
        ...
        bot.send_message(
            chat_id,
            f"💳 **تفاصيل الدفع عبر تطبيق جيب (Jeep)**\n\n"
            f"💵 المبلغ المطلوب: {amount}\n"
            f"📌 تم تسجيل طلبك بنجاح بانتظار التحويل.\n\n"
            f"⚡ **قم بتحويل المبلغ الآن عبر تطبيق جيب، وسيتم شحن رصيدك تلقائياً فور وصول إشعار التحويل!**",
            reply_markup=markup,
            parse_mode="Markdown"
        )
    except ValueError:
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("🔙 رجوع", callback_data="recharge_menu"))
        msg = bot.send_message(chat_id, "❌ خطأ: يرجى إرسال رقم صحيح للمبلغ:", reply_markup=markup)
        bot.register_next_step_handler(msg, process_jeep_amount)
...
def handle_user_steps(message):
    ...
    if step in ["waiting_amount", "waiting_txid", "waiting_jeep_amount"]:
        return
    ...
    if step == 'TRANSFER_TARGET':
        target_id_text = message.text.strip()
        if not target_id_text.isdigit():
            bot.send_message(message.chat.id, "❌ الآيدي يجب أن يكون أرقاماً فقط. يرجى إرسال آيدي المستلم الصحيح:", reply_markup=back_button())
            return
        
        target_id = int(target_id_text)
        if target_id == user_id:
            bot.send_message(message.chat.id, "❌ لا يمكنك تحويل رصيد لنفسك!", reply_markup=back_button())
            del USER_STEPS[user_id]
            return
            
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("SELECT user_id, name FROM users WHERE user_id = ?", (target_id,))
        target_user = cursor.fetchone()
        conn.close()
        
        if not target_user:
            bot.send_message(message.chat.id, "❌ المستخدم غير مسجل في البوت. تأكد من الآيدي وأعد المحاولة:", reply_markup=back_button())
            return
            
        USER_STEPS[user_id] = {'step': 'TRANSFER_AMOUNT', 'target_id': target_id, 'target_name': target_user[1]}
        bot.send_message(message.chat.id, f"👤 تم العثور على المستخدم: **{target_user[1]}** (`{target_id}`)\n\n💵 أرسل المبلغ المراد تحويله (بالدولار):\nأقل مبلغ للتحويل: $1.00", parse_mode="Markdown", reply_markup=back_button())
        return
        ...
        try:
            amount = float(message.text.strip())
            if amount < 1.00:
                bot.send_message(message.chat.id, "❌ أقل مبلغ للتحويل هو $1.00. أرسل مبلغاً صحيحاً:", reply_markup=back_button())
                return
        except ValueError:
            bot.send_message(message.chat.id, "❌ يرجى إرسال رقم صحيح للمبلغ:", reply_markup=back_button())
            return
        ...
        if sender_balance < amount:
            bot.send_message(message.chat.id, f"❌ رصيدك غير كافٍ لإتمام عملية التحويل.\n💰 رصيدك الحالي: ${sender_balance:.2f}", reply_markup=back_button())
            conn.close()
            del USER_STEPS[user_id]
            return
        ...
        bot.send_message(message.chat.id, f"✅ **تم تحويل الرصيد بنجاح!**\n\n👤 إلى: {target_name} (`{target_id}`)\n💵 المبلغ: `${amount:.2f}`", parse_mode="Markdown", reply_markup=back_button())
        try:
            bot.send_message(target_id, f"📥 **استلمت تحويل رصيد جديد!**\n\n💵 المبلغ: `${amount:.2f}`\n👤 من المستخدم: `{user_id}`")
        except:
            pass
        return
...
set_bot_commands()
print("Bot is running smoothly with Jeep automated structure and manual other wallets...")
