...
def get_main_welcome_text(user_id, name="", username="", balance=0.0):
    ...
    line1 = f"{rlm}<a href=\"{ch_off}\">قناة البوت الرسمية 📎</a>    {ltr}💙 {user_disp} 💙{ltr}"
    line2 = f"{rlm}<a href=\"{ch_ord}\">قناة التفعيلات 🛒</a>        {ltr}👤 : <code>{user_id}</code> •{ltr}"
    line3 = f"{rlm}<a href=\"{ch_tut}\">قناة شرح الاستخدام 📚</a>    {ltr}💲 : {bal_str}$ •{ltr}"
