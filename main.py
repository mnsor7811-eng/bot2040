...
def get_main_welcome_text(user_id, name="", username="", balance=0.0):
    ...
    target_width = 49
    # السطر الثاني: الآيدي (المرجع المثالي في أقصى اليسار)
    sp2_count = max(2, target_width - len(ch1_name) - len(id_vis))
    sp2 = " " * sp2_count

    # السطر الأول: اليوزر (إضافة 6 مسافات لدفعه لليسار تماماً بمحاذاة الآيدي دون كسر السطر)
    sp1_count = max(2, target_width - len(usr_vis) + 6)
    sp1 = " " * sp1_count

    # السطر الثالث: الرصيد (إضافة 3 مسافات لدفعه لليسار تماماً بمحاذاة الآيدي)
    sp3_count = max(2, target_width - len(ch2_name) - len(bal_part) + 10)
    sp3 = " " * sp3_count
