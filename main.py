...
def callback_handler(call):
    ...
    # ==================== معالجة أزرار شحن الرصيد وبايننس وجيب (Jeep) ====================
        ...
        msg = bot.send_message(
            ...
            "💵 أرسل المبلغ الذي تريد شحنه (بالدولار أو العملة المعتمدة):\n"
            "مثال: 5",
            reply_markup=markup
        )
...
def process_jeep_amount(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            f"⚡ **قم بتحويل المبلغ الآن عبر تطبيق جيب إلى حسابك/رقمك المعتمد، وسيتم شحن رصيدك تلقائياً فور وصول إشعار التحويل!**",
            reply_markup=markup,
            parse_mode="Markdown"
        )
    except ValueError:
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("🔙 رجوع", callback_data="recharge_menu"))
        msg = bot.send_message(chat_id, "❌ خطأ: يرجى إرسال رقم صحيح للمبلغ (مثال: 5 أو 10):", reply_markup=markup)
        bot.register_next_step_handler(msg, process_jeep_amount)
...
set_bot_commands()
print("Bot is running smoothly with Full Menu Commands and Jeep Webhook support...")
