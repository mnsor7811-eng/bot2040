...
from keyboards import (
    ...
    admin_smm_services_list_keyboard, admin_smm_guarantee_keyboard,
...
def callback_handler(call):
    ...
    # إدارة خدمات الرشق المخصصة (SMM Custom Services)
        ...
        try: bot.edit_message_text(success_text, chat_id, message_id, parse_mode="Markdown", reply_markup=admin_smm_custom_keyboard())
        except: bot.send_message(chat_id, success_text, parse_mode="Markdown", reply_markup=admin_smm_custom_keyboard())
        return
...
def handle_user_steps(message):
    ...
    if user_id == ADMIN_ID:
        ...
        # خطوات إضافة خدمة رشق مخصصة جديدة للأدمن (SMM Custom Service)
            ...
            if rate <= 0:
                bot.send_message(message.chat.id, "❌ يرجى إرسال سعر صالح (مثال: `0.85` أو `0.10`):", reply_markup=admin_back_button())
                return
            step_data['rate'] = rate
            ...
            confirm_prompt = (
                f"🛡️ **اختر نوع الضمان لإتمام إضافة الخدمة:**\n\n"
                f"▫️ **القسم** : {cat_title}\n"
                f"▫️ **الاسم** : {name_ar}\n"
                f"▫️ **ID السيرفر** : `{srv_api_id}`\n"
                f"▫️ **السعر** : `${rate:.3f}` لكل 1K\n"
                f"▫️ **الكمية** : من `{min_q}` إلى `{max_q}`\n\n"
                f"اضغط على نوع الضمان بالأسفل لحفظ الخدمة فوراً:"
            )
            bot.send_message(message.chat.id, confirm_prompt, parse_mode="Markdown", reply_markup=admin_smm_guarantee_keyboard())
            return
