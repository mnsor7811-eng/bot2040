...
from config import (
    ...
    get_custom_smm_services, add_custom_smm_service, delete_custom_smm_service
)
...
from keyboards import (
    ...
    admin_smm_custom_keyboard, admin_smm_services_list_keyboard,
...
def get_cached_smm_services(server_id='2'):
    ...
    # أولاً: جلب أي خدمات مخصصة مدخلة يدوياً من قبل الأدمن لهذا السيرفر
    ...
    # دمج الخدمات المخصصة مع خدمات الـ API (تكون الخدمات المخصصة في البداية)
    if custom_services:
...
def filter_smm_services(target_type, server_id='2'):
    ...
    for srv in services:
        # إذا كانت الخدمة مخصصة يدوياً وتطابق التصنيف مباشرة
...
def check_subscription(user_id):
    ...
    # جلب قنوات الاشتراك الإجباري من الإعدادات
    ...
    channels_to_check = []
    for c_id in [ch_orders_id, ch_official_id]:
        if c_id:
            try:
                channels_to_check.append(int(c_id))
            except:
                channels_to_check.append(str(c_id))
...
def subscription_markup():
    ch_orders_url = get_setting('channel_orders_url', CHANNEL_ORDERS_URL)
    ch_official_url = get_setting('channel_official_url', CHANNEL_OFFICIAL_URL)
    ch_orders_name = get_setting('channel_orders_name', CHANNEL_ORDERS_NAME)
    ch_official_name = get_setting('channel_official_name', CHANNEL_OFFICIAL_NAME)
    
    markup = InlineKeyboardMarkup()
    if ch_orders_url:
        markup.add(InlineKeyboardButton(f"{ch_orders_name} 📢", url=ch_orders_url))
    if ch_official_url:
        markup.add(InlineKeyboardButton(f"{ch_official_name} 🤖", url=ch_official_url))
...
def notify_smm_activation(order_id, service_name, category_name, quantity, price, user_id):
    """إشعار فوري عند تنفيذ طلب رشق ودعم فوري"""
    ...
    text = (
        ...
        f"⏰ <b>الوقت</b> : {now_str}\n"
        f"✅ <b>الحالة</b> : قيد التنفيذ المباشر ⚡\n"
        f"━━━━━━━━━━━━━━━━━━━━"
    )
...
def callback_handler(call):
    ...
    # إدارة خدمات الرشق المخصصة (SMM Custom Services)
        ...
        if user_id != ADMIN_ID: return
        msg_custom = (
            "⚡ **إدارة خدمات الرشق المخصصة (سيرفر Tiger / سيرفرات أخرى):**\n\n"
            "▫️ يمكنك إضافة أي خدمة يدوياً باللغة العربية مع تحديد السعر بالدولار لكل 1000 متابع/تفاعل، والحد الأدنى والأقصى.\n"
            "▫️ ستظهر الخدمة للمستخدمين بنفس الواجهة الاحترافية والتنفيذ الفوري والتلقائي عبر السيرفر!"
        )
    ...
    elif call.data == "adm_smm_add":
        if user_id != ADMIN_ID: return
        USER_STEPS[user_id] = {'step': 'ADM_ADD_SMM_1'}
        add_text = (
            "➕ **إضافة خدمة رشق جديدة يدوياً**\n\n"
            "📌 **الخطوة 1 من 6:**\n"
            "أرسل **رقم ID الخدمة في موقع المزود (SMM Tigers Service ID)**:\n"
            "(مثال: `142` أو `250`)"
        )
        bot.send_message(chat_id, add_text, parse_mode="Markdown", reply_markup=admin_back_button())
        return

    elif call.data.startswith("adm_smm_list_"):
        if user_id != ADMIN_ID: return
        page = int(call.data.split("_")[3])
        try: bot.edit_message_text("📋 **قائمة الخدمات المضافة يدوياً:**", chat_id, message_id, parse_mode="Markdown", reply_markup=admin_smm_services_list_keyboard(page=page))
        except: bot.send_message(chat_id, "📋 **قائمة الخدمات المضافة يدوياً:**", parse_mode="Markdown", reply_markup=admin_smm_services_list_keyboard(page=page))
        return
        ...
        try: bot.edit_message_text("📋 تم تحديث قائمة الخدمات:", chat_id, message_id, reply_markup=admin_smm_services_list_keyboard(0))
        except: pass
        return
        ...
        view_text = (
            ...
            f"▫️ **المعرف في الموقع (Service ID)** : `{srv.get('service')}`\n"
            f"▫️ **السيرفر** : `{srv.get('server_id')}`\n"
            f"▫️ **القسم / التطبيق** : `{srv.get('category')}`\n"
            f"▫️ **السعر لكل 1000** : `${srv.get('rate')}`\n"
            f"▫️ **الحد الأدنى** : `{srv.get('min')}`\n"
            f"▫️ **الحد الأقصى** : `{srv.get('max')}`"
        )
        markup = InlineKeyboardMarkup()
        markup.row(InlineKeyboardButton("🗑️ حذف هذه الخدمة", callback_data=f"adm_smm_del_{db_id}"))
        markup.row(InlineKeyboardButton("🔙 رجوع للقائمة", callback_data="adm_smm_list_0"))
    ...
    # 10. مخزون الحسابات القديمة (سيرفر 3)
        ...
        msg = (
            ...
            f"💵 **إجمالي المبيعات (Sales)** : `${total_sales:.2f}`\n"
            f"📦 **إجمالي تكلفة المزودين (Cost)** : `${total_cost:.2f}`\n"
            f"💎 **صافي الأرباح الفعلية (Net Profit)** : `${net_profit:.2f}`\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    # ==================== باقي معالجات الأزرار القياسية ====================
        ...
        user_display = f"@{call.from_user.username}" if call.from_user.username else (call.from_user.first_name or str(user_id))
    ...
    # ==================== أقسام الرشق (SMM) ====================
        ...
        try:
            ...
            if order_response and 'order' in order_response:
                ...
                notify_smm_activation(order_id, service_name, category_name, qty, total_cost, user_id)
        ...
        ref_msg = (
            ...
            f"🎁 **مكافأة كل دعوة** : `${rew:.2f}` تضاف فور انضمام العضو عبر رابطك!\n"
            f"🏧 **الحد الأدنى للتحويل إلى رصيدك** : `${min_w:.2f}`\n\n"
            f"🔗 **رابط إحالتك الخاص** :\n`{ref_link}`\n\n"
            f"👥 **إجمالي المدعوين** : `{ref_cnt}` مستخدم\n"
            f"💵 **أرباحك المتجمعة** : `${ref_earn:.2f}`\n\n"
            f"📌 انشر رابطك في المجموعات والقنوات، وعند بلوغ الحد الأدنى اضغط على زر [ تحويل الأرباح إلى رصيد ] أدناه ليتم شحن رصيدك فوراً وشراء الأرقام والخدمات مجاناً!"
        )
        
        markup = InlineKeyboardMarkup()
        if ref_earn >= min_w:
            markup.add(InlineKeyboardButton(f"🔄 تحويل الأرباح (${ref_earn:.2f}) إلى رصيدي بالبوت", callback_data="withdraw_ref_earnings"))
        ...
        try:
            ...
            bot.send_message(chat_id, f"🎉 **تم تحويل أرباح الإحالة إلى رصيدك بنجاح!**\n\n💵 تم إضافة `${ref_earn:.2f}` إلى رصيدك الأساسي في البوت.\n💰 يمكنك الآن استخدامها لشراء الأرقام والحسابات والخدمات فوراً.", parse_mode="Markdown", reply_markup=back_button())
        finally:
            conn.close()
        return
...
def handle_user_steps(message):
    ...
    if user_id == ADMIN_ID:
        ...
        # خطوات إضافة خدمة رشق مخصصة جديدة للأدمن (SMM Custom Service)
        elif step == 'ADM_ADD_SMM_1':
            srv_api_id = message.text.strip()
            step_data['srv_api_id'] = srv_api_id
            step_data['step'] = 'ADM_ADD_SMM_2'
            cat_text = (
                f"✅ تم حفظ ID الخدمة: `{srv_api_id}`\n\n"
                f"📌 **الخطوة 2 من 6:**\n"
                f"أرسل كود القسم / التطبيق للخدمة:\n"
                f"▫️ `telegram` (تيليجرام)\n"
                f"▫️ `instagram` (إنستغرام)\n"
                f"▫️ `tiktok` (تيك توك)\n"
                f"▫️ `youtube` (يوتيوب)\n"
                f"▫️ `facebook` (فيسبوك)\n"
                f"▫️ `twitter` (تويتر / X)\n"
            ...
            bot.send_message(message.chat.id, cat_text, parse_mode="Markdown", reply_markup=admin_back_button())
            return

        elif step == 'ADM_ADD_SMM_2':
            cat_code = message.text.strip().lower()
            step_data['cat_code'] = cat_code
            step_data['step'] = 'ADM_ADD_SMM_3'
            bot.send_message(message.chat.id, f"📌 **الخطوة 3 من 6:**\nأرسل **اسم الخدمة باللغة العربية كاملاً** كما تريده أن يظهر للمستخدم:\n(مثال: `👑 أعضاء قنوات تيليجرام حقيقيين ضمان 30 يوم`):", reply_markup=admin_back_button())
            return

        elif step == 'ADM_ADD_SMM_3':
            name_ar = message.text.strip()
            step_data['name_ar'] = name_ar
            step_data['step'] = 'ADM_ADD_SMM_4'
            bot.send_message(message.chat.id, f"📌 **الخطوة 4 من 6:**\nأرسل **سعر بيع الخدمة لكل 1000 بالدولار ($)**:\n(مثال: `0.85` أو `1.2` أو `0.35`):", reply_markup=admin_back_button())
            return

        elif step == 'ADM_ADD_SMM_4':
            try:
                rate = float(message.text.strip())
                step_data['rate'] = rate
                step_data['step'] = 'ADM_ADD_SMM_5'
                bot.send_message(message.chat.id, f"📌 **الخطوة 5 من 6:**\nأرسل **الحد الأدنى للطلب (Min)**:\n(مثال: `10` أو `50` أو `100`):", reply_markup=admin_back_button())
            except:
                bot.send_message(message.chat.id, "❌ أرسل رقماً صحيحاً للسعر (مثال: `0.85`).", reply_markup=admin_back_button())
            return

        elif step == 'ADM_ADD_SMM_5':
            try:
                min_q = int(message.text.strip())
                step_data['min_q'] = min_q
                step_data['step'] = 'ADM_ADD_SMM_6'
                bot.send_message(message.chat.id, f"📌 **الخطوة 6 من 6:**\nأرسل **الحد الأقصى للطلب (Max)**:\n(مثال: `10000` أو `50000` أو `100000`):", reply_markup=admin_back_button())
            except:
                bot.send_message(message.chat.id, "❌ أرسل رقماً صحيحاً للحد الأدنى (مثال: `50`).", reply_markup=admin_back_button())
            return

        elif step == 'ADM_ADD_SMM_6':
            try:
                ...
                success_text = (
                    ...
                    f"▫️ **ID المزود (Tiger)** : `{srv_api_id}`\n"
                    f"▫️ **القسم** : `{cat_code}`\n"
                    f"▫️ **السعر لكل 1K** : `${rate:.3f}`\n"
                    f"▫️ **الحد الأدنى** : `{min_q}` | **الحد الأقصى** : `{max_q}`\n\n"
                    f"✅ أصبحت الخدمة متاحة ومباشرة للمستخدمين في سيرفر الرشق 2 (SMM Tigers)!"
                )
                bot.send_message(message.chat.id, success_text, parse_mode="Markdown", reply_markup=admin_smm_custom_keyboard())
            except Exception as e:
                bot.send_message(message.chat.id, f"❌ حدث خطأ أثناء الإضافة: {e}", reply_markup=admin_back_button())
            return
