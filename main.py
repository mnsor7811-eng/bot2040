...
def get_main_welcome_text(user_id, name="", username="", balance=0.0):
    ...
    target_width = 59
    sp_id_count = max(2, target_width - len(id_vis))
    sp_id = " " * sp_id_count

    # السطر الثاني (الرصيد): تعويض فرق عرض الأرقام عن المسافات ليكون على نفس الخط العمودي للآيدي
    diff = len(id_vis) - len(bal_part)
    sp_bal_count = max(2, sp_id_count + int(diff * 1.9))
    sp_bal = " " * sp_bal_count
    ...
    # السطر 1: اليسار: الآيدي فقط
    # السطر 2: اليسار: الرصيد فقط (ثابت ومحاذي للآيدي دائماً مهما كبر المبلغ)
    # السطر 3+: اليمين: القنوات تبدأ من السطر الثالث بمفردها دون أي نص في المقابل
    line1 = f"{rlm}{sp_id}{ltr}{id_part}{ltr}"
    line2 = f"{rlm}{sp_bal}{ltr}{bal_part}{ltr}"
    line3 = f"{rlm}<a href=\"{ch_off}\">{rlm}{ch1_name}</a>"
    line4 = f"{rlm}<a href=\"{ch_ord}\">{rlm}{ch2_name}</a>"
    line5 = f"{rlm}<a href=\"{ch_tut}\">{rlm}{ch3_name}</a>"
    line6 = f"{rlm}<a href=\"{ch_exp}\">{rlm}{ch4_name}</a>"
