...
def start_cmd(message):
    ...
    # التأكد من وجود عمود username في الجدول
    try:
    ...
    if not user:
        ...
        # تحديث الاسم واليوزر نيم باستمرار
...
def callback_handler(call):
    ...
    if call.data == "admin_panel":
        ...
        bot.send_message(chat_id, "📋 **قائمة أحدث 10 مستخدمين مسجلين:**", parse_mode="Markdown")
        for u in users:
            u_id, u_name, u_uname, u_bal, u_ai, u_ban = u
            u_uname = u_uname if u_uname else "لا يوجد"
    ...
    # إجراءات سريعة فورية للمستخدم المكتشف
...
def handle_user_steps(message):
    ...
    if str(user_id) == str(ADMIN_ID):
        if step == 'ADMIN_BROADCAST':
            ...
            cursor.execute("SELECT user_id, name, username, balance, ai_balance, is_banned FROM users WHERE name LIKE ? OR username LIKE ? OR user_id = ?", 
                           ('%' + query + '%', '%' + query + '%', query))
            users = cursor.fetchall()
            conn.close()
            ...
            bot.send_message(message.chat.id, f"🔍 **تم العثور على ({len(users)}) نتيجة:**", parse_mode="Markdown")
            for u in users:
                u_id, u_name, u_uname, u_bal, u_ai, u_ban = u
                u_uname = u_uname if u_uname else "لا يوجد"
                status = "محظور 🚫" if u_ban == 1 else "نشط ✅"
                ...
                # أزرار تحكم مخصصة ومباشرة لكل مستخدم ظهر في البحث
                mk = InlineKeyboardMarkup()
                mk.row(InlineKeyboardButton("➕ شحن رصيد", callback_data=f"act_add_{u_id}"), InlineKeyboardButton("➖ خصم رصيد", callback_data=f"act_deduct_{u_id}"))
                mk.row(InlineKeyboardButton("🤖 شحن AI", callback_data=f"act_ai_{u_id}"), InlineKeyboardButton("🚫 حظر / فك", callback_data=f"act_ban_{u_id}"))
                bot.send_message(message.chat.id, res, parse_mode="Markdown", reply_markup=mk)
            return
