...
def get_main_welcome_text(user_id, name="", username="", balance=0.0):
    ...
    # حساب المسافات باستخدام مسافات عريضة (EM Spaces) لدفع البيانات إلى أقصى اليسار تماماً
    em = "\u2003"
    
    # حساب العرض التقريبي لكل عنصر بوحدات EM
    def get_em_w(s):
        w = 0.0
        for c in s:
            if ord(c) > 0x1F000 or ord(c) in [0x2600, 0x2700, 0x231A]:
                w += 1.0  # إيموجي
            elif '\u0600' <= c <= '\u06FF':
                w += 0.55 # حرف عربي
            else:
                w += 0.5  # أرقام ورموز ولاتيني
        return w

    em_target = 21.0
    w_usr = get_em_w(usr_vis)
    w_ch1 = get_em_w(ch1_name)
    w_id = get_em_w(id_vis)
    w_ch2 = get_em_w(ch2_name)
    w_bal = get_em_w(bal_part)

    sp1 = em * max(2, int(round(em_target - w_usr)))
    sp2 = em * max(2, int(round(em_target - w_ch1 - w_id)))
    sp3 = em * max(2, int(round(em_target - w_ch2 - w_bal)))
