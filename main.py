...
def start_cmd(message):
    ...
    # فحص نظام التحقق الأمني الذكي (Anti-Bot)
    ...
    if captcha_active and user_id != ADMIN_ID:
        ...
        try:
            ...
            is_v = v_row[0] if (v_row and v_row[0] is not None) else None
        finally:
            conn.close()

        if is_v == 0 or is_v is None:
            import random
...
def callback_handler(call):
    ...
    if user_id != ADMIN_ID and get_setting('captcha_enabled', '1') == '1':
        if call.data not in ["check_sub"]:
            ...
            try:
                cursor.execute('SELECT is_verified FROM users WHERE user_id = ?', (user_id,))
                v_row = cursor.fetchone()
                is_v = v_row[0] if (v_row and v_row[0] is not None) else 0
            finally:
                conn.close()
            if is_v == 0:
    ...
    # ==================== أقسام الرشق (SMM) ====================
        ...
        try: bot.edit_message_text(stats_text, chat_id, message_id, parse_mode="HTML", reply_markup=markup)
        except: bot.send_message(chat_id, stats_text, parse_mode="HTML", reply_markup=markup)
        return
        ...
        if message.text.strip() == str(expected).strip():
            ...
            try:
                ...
                if not user:
                    ...
                    cursor.execute('UPDATE users SET name = ?, username = ?, is_verified = 1 WHERE user_id = ?', (name, username, user_id))
                    conn.commit()
            finally:
                conn.close()
