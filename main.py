...
def callback_handler(call):
    ...
    # ==================== الدفع عبر Binance (تلقائي) ====================
        ...
        if pay_info:
            ...
            try: bot.edit_message_text(msg, chat_id, message_id, reply_markup=back_markup, parse_mode="Markdown")
            except Exception:
                try: bot.delete_message(chat_id, message_id)
                except: pass
                bot.send_message(chat_id, msg, reply_markup=back_markup, parse_mode="Markdown")
        return
    ...
    # ==================== باقي معالجات الأزرار القياسية ====================
        ...
        try:
            ...
            if "ACCESS_NUMBER" in res:
                ...
                try: bot.edit_message_text(f"❌ **لم يكتمل الطلب:**\n{err_msg}", chat_id, message_id, parse_mode="Markdown", reply_markup=back_button())
                except: bot.send_message(chat_id, f"❌ **لم يكتمل الطلب:**\n{err_msg}", parse_mode="Markdown", reply_markup=back_button())
        finally:
            conn.close()
        ...
        try:
            ...
            # إعادة الرصيد للمستخدم فوراً وإلغاء الطلب محلياً دون أي تأخير أو تعليق
            cursor.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', (cost, user_id))
            ...
            res_cancel = grizzly_request({'action': 'setStatus', 'status': '8', 'id': tz_id}, srv['api_key'], srv['url'])
            if "EARLY_CANCEL_DENIED" in res_cancel:
                # وضع في قائمة الانتظار للمزود لاسترداد رصيد الأدمن في المزود فور انتهاء التوقيت دون خصم أو إضافة أخرى للمستخدم
                cursor.execute('INSERT OR REPLACE INTO pending_cancellations (tz_id, server_id, user_id, chat_id, cost, phone, refund_user) VALUES (?, ?, ?, ?, ?, ?, 0)',
