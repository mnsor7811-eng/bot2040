...
def mask_user(user_id):
    if not user_id: return "عميل متميز"
    s = str(user_id).strip()
    if len(s) >= 8:
        return f"{s[:6]}××××"
    elif len(s) >= 4:
...
def send_to_channel_safe(text, reply_markup=None):
    """إرسال آمن لقناة التفعيلات مع تجربة عدة معرفات لتفادي أي أخطاء"""
    targets = []
    
    # 1. المعرف من الرابط المخزن في الإعدادات
    ...
    # 2. المعرف من معرف القناة الرقمي
    ch_id_str = get_setting('channel_orders_id', CHANNEL_ORDERS_ID)
    if ch_id_str:
        clean_id = str(ch_id_str).strip()
        if clean_id.startswith('-100'):
            targets.append(clean_id)
        elif clean_id.isdigit():
            targets.append(f"-100{clean_id}")
            targets.append(clean_id)
        elif clean_id.startswith('@'):
            targets.append(clean_id)
            
    # 3. معرفات افتراضية موثوقة
    if "@numbuersms" not in targets:
        targets.append("@numbuersms")
    if "-1002987190358" not in targets:
        targets.append("-1002987190358")
...
def notify_number_activation(order_id, country_name, country_flag, server_name, service_name, phone, code, price, user_id):
    """إشعار فوري فخم عند تفعيل رقم وهمي واستلام الكود بنجاح"""
    ...
    text = (
        f"<b>🌟 Number-sms sales | التفعيلات الناجحة</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🔔 <b>رقم الطلب</b> : <code>{masked_p}</code>\n"
        f"🌐 <b>الدولة</b> : {country_name} {country_flag}\n"
        f"👍 <b>التفعيل</b> : آلي / فوري ✅\n"
        f"🛍️ <b>السيرفر</b> : {server_name}\n"
        f"📱 <b>المنصة</b> : {service_name}\n"
        f"🆔 <b>العميل</b> : <code>{masked_u}</code>\n"
        f"💙 <b>السعر</b> : ${float(price):.2f}\n"
        f"📬 <b>وقت الاستلام</b> : {now_str}\n"
        f"✅ <b>الحالة</b> : تم التفعيل بنجاح\n\n"
        f"📨 <b>رقم الرسالة</b> : 1\n"
        f"📩 <b>الاستلام</b> : {now_str}\n"
        f"➕ <b>المرسل</b> : {service_name}\n"
        f"👁️ <b>كود التفعيل</b> : <code>{masked_c}</code>\n"
        f"━━━━━━━━━━━━━━━━━━━━"
    )
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("• ✓ طلب رقم من السيرفر •", url=f"https://t.me/{bot_uname}"))
    
    send_to_channel_safe(text, reply_markup=markup)

def notify_ready_account_activation(order_id, country_name, phone, code, password, price, user_id, server_name="سيرفر الحسابات 1"):
    """إشعار فوري عند تفعيل وتسليم حساب تيليجرام جاهز واستلام الكود"""
    now_str = datetime.datetime.now().strftime("%Y/%m/%d %H:%M")
    masked_p = mask_phone(phone)
    masked_c = mask_code(code)
    masked_u = mask_user(user_id)
    ...
    text = (
        f"<b>🎉 تم تفعيل وتسليم حساب تيليجرام جاهز!</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🔔 <b>رقم الطلب</b> : <code>#TG-{str(order_id)[:8]}</code>\n"
        f"🌐 <b>الدولة</b> : {country_name}\n"
        f"📱 <b>المنصة</b> : تيليجرام (حساب جاهز ومفعل 100%)\n"
        f"📞 <b>الرقم</b> : <code>{masked_p}</code>\n"
        f"🆔 <b>العميل</b> : <code>{masked_u}</code>\n"
        f"💵 <b>السعر</b> : ${float(price):.2f}\n"
        f"⏰ <b>وقت التفعيل</b> : {now_str}\n"
        f"✅ <b>الحالة</b> : تم الدخول واستلام الكود بنجاح\n\n"
        f"🔑 <b>كود التفعيل</b> : <code>{masked_c}</code>\n"
        f"🔐 <b>الحماية (2FA)</b> : مفعل ومؤمن بنجاح 🛡️\n"
        f"━━━━━━━━━━━━━━━━━━━━"
    )
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("• 🛍️ شراء حساب تيليجرام الآن •", url=f"https://t.me/{bot_uname}"))
...
def notify_channel_order(service_type, title, price, user_id=None, details=""):
    ...
    text = (
        f"<b>🎉 {type_title}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🛍️ <b>الخدمة</b> : {title}\n"
        f"👤 <b>العميل</b> : <code>{masked_user}</code>\n"
        f"💵 <b>المبلغ</b> : ${float(price):.2f}\n"
        f"⏰ <b>التاريخ</b> : {now_str}\n"
    )
    if details:
        text += f"📌 <b>تفاصيل</b> : {details}\n"
...
def callback_handler(call):
    ...
    # ==================== حسابات تيليجرام جاهزة ====================
        ...
        if code_res.get('ok') and code_res.get('code'):
            ...
            # استرجاع تفاصيل الحساب لإرسال الإشعار لقناة التفعيلات
            c_name_val = "تيليجرام"
            cost_val = 0.0
            phone_full = num_val
            try:
                conn = get_db()
                cursor = conn.cursor()
                cursor.execute("SELECT country_name, cost, phone FROM ready_accounts_orders WHERE session_file = ? OR phone = ? ORDER BY id DESC LIMIT 1", (str(lookup_key), str(num_val)))
                row = cursor.fetchone()
                if row:
                    c_name_val = row[0] or "تيليجرام"
                    cost_val = float(row[1] or 0.0)
                    phone_full = row[2] or num_val
                conn.close()
            except: pass
            ...
            notify_ready_account_activation(
                ...
                price=cost_val,
                user_id=user_id,
                server_name=f"سيرفر الحسابات {srv_id}"
            )
    ...
    # 5. إدارة القنوات
        ...
        if ok:
            ...
            bot.send_message(
                ...
                f"2️⃣ اضغط **إضافة مشرف** وابحث عن يوزر البوت `@NUM_SMBOT` وقم بإضافته مع تفعيل صلاحية **نشر الرسائل (Post Messages)**.\n"
    ...
    # ==================== باقي معالجات الأزرار القياسية ====================
        ...
        try: bot.edit_message_text(f"🌐 اختر الدولة المطلوبة لـ {srv_display}:", chat_id, message_id, reply_markup=markup)
        except: pass
        ...
        try:
            ...
            # إشعار سريع للمستخدم بالبدء
            bot.answer_callback_query(call.id, "⚡ جاري جلب رقم بديل جديد فوراً...")

            # محاولة إلغاء الرقم القديم في المزود
            res_cancel = grizzly_request({'action': 'setStatus', 'status': '8', 'id': tz_id}, srv['api_key'], srv['url'])
            if "EARLY_CANCEL_DENIED" in res_cancel:
                # إدراج الرقم القديم في قائمة الإلغاء التلقائي في الخلفية لكي يتم إلغاؤه فور انتهاء العداد واسترجاع رصيدك في المزود
            ...
            # طلب رقم جديد فـوراً ومباشرة لنفس الدولة والتطبيق دون أي تعطيل للزبون
            res = grizzly_request({'action': 'getNumber', 'service': srv_code, 'country': country_code}, srv['api_key'], srv['url'])
            if "ACCESS_NUMBER" in res:
                p_parts = res.split(":")
                ...
                # في حال عدم توفر رقم جديد، يتم إرجاع المبلغ فوراً لحساب الزبون
        ...
        try:
            ...
            if "ACCESS_CANCEL" in res_cancel or "STATUS_CANCEL" in res_cancel:
                cursor.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', (cost, user_id))
            ...
            elif "EARLY_CANCEL_DENIED" in res_cancel:
                cursor.execute('INSERT OR REPLACE INTO pending_cancellations (tz_id, server_id, user_id, chat_id, cost, phone, refund_user) VALUES (?, ?, ?, ?, ?, ?, 1)',
                               (tz_id, server_id, user_id, chat_id, cost, phone, 1))
                conn.commit()
                bot.edit_message_text(
                    f"⏳ **موقع المزود يفرض فترة انتظار قبل الإلغاء**\n"
                    f"(متاح الإلغاء بعد دقيقتين إلى 4 دقائق لحماية السيرفرات).\n\n"
                    f"🤖 **تم تفعيل الإلغاء التلقائي بنجاح!**\n"
                    f"يقوم البوت الآن بمحاولة إلغاء الرقم `{phone}` تلقائياً في الخلفية فور فتح الإلغاء، وسيقوم بإعادة المبلغ (${cost:.2f}) إلى رصيدك وإشعارك هنا فوراً.",
                    chat_id, message_id, parse_mode="Markdown", reply_markup=back_button()
                )
            else:
                bot.answer_callback_query(call.id, f"استجابة المزود: {res_cancel}", show_alert=True)
        finally:
            conn.close()
    ...
    # ==================== أقسام الرشق (SMM) ====================
        ...
        bot.edit_message_text("🔵 عروض Telegram المتاحة:", chat_id, message_id, reply_markup=markup)
...
def process_pending_cancellations():
    """فحص دوري في الخلفية كل 15 ثانية لإلغاء الأرقام فور انتهاء عداد الانتظار في المزود"""
    while True:
        try:
            time.sleep(15)
            conn = get_db()
            cursor = conn.cursor()
            cursor.execute("SELECT tz_id, server_id, user_id, chat_id, cost, phone, refund_user FROM pending_cancellations")
            pending_list = cursor.fetchall()
            for row in pending_list:
                tz_id, server_id, user_id, chat_id, cost, phone, refund_user = row
                srv = SERVERS.get(server_id)
                if not srv:
                    srv = {'api_key': API_KEY, 'url': API_URL}
                
                res = grizzly_request({'action': 'setStatus', 'status': '8', 'id': tz_id}, srv['api_key'], srv['url'])
                # إذا نجح الإلغاء أو إذا انتهت مهلة الرقم في المزود
                if "ACCESS_CANCEL" in res or "STATUS_CANCEL" in res or "BAD_STATUS" in res or "NO_ACTIVATION" in res:
                    if refund_user == 1:
                        cursor.execute("UPDATE users SET balance = balance + ? WHERE user_id = ?", (cost, user_id))
                        cursor.execute("UPDATE purchases SET status = 'CANCELLED' WHERE tz_id = ?", (tz_id,))
                        try:
