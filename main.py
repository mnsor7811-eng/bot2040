...
def get_main_welcome_text(user_id, name="", username="", balance=0.0):
    ...
    # حساب المسافات بدقة لضبط المحاذاة في أقصى اليسار تماماً مثل الآيدي
    target_width = 49
    # السطر الثاني (الآيدي) هو المرجع الأساسي:
    sp2_count = max(2, target_width - len(ch1_name) - len(id_vis))
    sp2 = " " * sp2_count

    # السطر الأول (اليوزر) لا يحتوي على قناة في اليمين، لذا يتم تعويض عرض القناة بالمسافات ليصل لأقصى اليسار:
    extra_ch1_spaces = 26
    usr_diff_spaces = int((len(id_vis) - len(usr_vis)) * 2.0)
    sp1_count = max(2, sp2_count + extra_ch1_spaces + usr_diff_spaces)
    sp1 = " " * sp1_count

    # السطر الثالث (الرصيد) أقصر من الآيدي، لذا يتم تعويض الفارق بالمسافات ليصل لنفس خط الآيدي في اليسار:
    bal_diff_spaces = int((len(id_vis) - len(bal_part)) * 2.0) - int((len(ch2_name) - len(ch1_name)) * 2.0)
    sp3_count = max(2, sp2_count + bal_diff_spaces + 2)
    sp3 = " " * sp3_count
