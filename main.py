...
def start_cmd(message):
    ...
    try:
        balance = float(user_data[2]) if user_data[2] is not None else 0.0
    except (ValueError, TypeError):
        balance = 0.0
        
    try:
        ai_bal = int(user_data[3]) if user_data[3] is not None else 5
    except (ValueError, TypeError):
        ai_bal = 5
