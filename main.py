...
def callback_handler(call):
    ...
    # ==================== أقسام الرشق (SMM) ====================
        ...
        if "سريع" in name or "fast" in name.lower() or "فوري" in name or "instant" in name.lower():
            speed = "فورية ⚡"
...
def handle_user_steps(message):
    ...
    if user_id == ADMIN_ID:
        ...
        # خطوات إضافة خدمة رشق مخصصة جديدة للأدمن (SMM Custom Service)
            ...
            try:
                bot.send_message(message.chat.id, confirm_prompt, reply_markup=admin_smm_guarantee_keyboard())
            except Exception as e:
