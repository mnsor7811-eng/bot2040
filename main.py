...
from keyboards import (
    ...
    ready_account_detail_keyboard, tg_servers_keyboard, recharge_keyboard,
...
# ==================== أوامر البدء ====================
@bot.message_handler(commands=['start', 'num', 'store'])
...
def callback_handler(call):
    ...
    if call.data == "store_menu":
        ...
        products = search_products_by_category("gift") or search_products_by_category("card")
        keyboard = InlineKeyboardMarkup(row_width=1)
        if products:
            for p in products[:15]:
                p_name = p.get('name', 'بطاقة هدايا')
                p_price = p.get('price', 0)
                p_id = p.get('id') or p.get('slug')
                keyboard.add(InlineKeyboardButton(f"🎁 {p_name} - ${p_price}", callback_data=f"buyprod_{p_id}"))
        keyboard.add(InlineKeyboardButton("🔙 عودة للمتجر", callback_data="store_menu"))
        try:
            bot.edit_message_text("🎁 **قائمة بطاقات الهدايا المتوفرة:**\nاختر المنتج الذي تود شراءه:", chat_id, message_id, reply_markup=keyboard, parse_mode="Markdown")
        except:
            bot.send_message(chat_id, "🎁 **قائمة بطاقات الهدايا المتوفرة:**\nاختر المنتج الذي تود شراءه:", reply_markup=keyboard, parse_mode="Markdown")
        return

    elif call.data == "cat_gaming":
        try: bot.answer_callback_query(call.id)
        except: pass
        products = search_products_by_category("game") or search_products_by_category("pubg")
        keyboard = InlineKeyboardMarkup(row_width=1)
        if products:
            for p in products[:15]:
                p_name = p.get('name', 'منتج لعبة')
                p_price = p.get('price', 0)
                p_id = p.get('id') or p.get('slug')
                keyboard.add(InlineKeyboardButton(f"🎮 {p_name} - ${p_price}", callback_data=f"buyprod_{p_id}"))
        keyboard.add(InlineKeyboardButton("🔙 عودة للمتجر", callback_data="store_menu"))
        try:
            bot.edit_message_text("🎮 **قائمة شحن الألعاب المتوفرة:**\nاختر المنتج الذي تود شراءه:", chat_id, message_id, reply_markup=keyboard, parse_mode="Markdown")
        except:
            bot.send_message(chat_id, "🎮 **قائمة شحن الألعاب المتوفرة:**\nاختر المنتج الذي تود شراءه:", reply_markup=keyboard, parse_mode="Markdown")
        return

    elif call.data == "cat_services":
        try: bot.answer_callback_query(call.id)
        except: pass
        products = search_products_by_category("service") or search_products_by_category("net")
        keyboard = InlineKeyboardMarkup(row_width=1)
        if products:
            for p in products[:15]:
                p_name = p.get('name', 'خدمة رقمية')
                p_price = p.get('price', 0)
                p_id = p.get('id') or p.get('slug')
                keyboard.add(InlineKeyboardButton(f"🌐 {p_name} - ${p_price}", callback_data=f"buyprod_{p_id}"))
        keyboard.add(InlineKeyboardButton("🔙 عودة للمتجر", callback_data="store_menu"))
        try:
            bot.edit_message_text("🌐 **خدمات الإنترنت والبرمجيات المتوفرة:**\nاختر المنتج الذي تود شراءه:", chat_id, message_id, reply_markup=keyboard, parse_mode="Markdown")
        except:
            bot.send_message(chat_id, "🌐 **خدمات الإنترنت والبرمجيات المتوفرة:**\nاختر المنتج الذي تود شراءه:", reply_markup=keyboard, parse_mode="Markdown")
        return

    elif call.data.startswith("buyprod_"):
        product_id = call.data.replace("buyprod_", "")
        bot.answer_callback_query(call.id, "⏳ جاري تنفيذ الطلب وخصم الرصيد تلقائياً...")
        
        # جلب بيانات المستخدم المالية للتحقق من الرصيد
        conn = get_db()
        cursor = conn.cursor()
        try:
        ...
        # تنفيذ الشراء التلقائي من الـ API
        result = create_and_pay_order(product_id, quantity=1)
        
        if result.get("success"):
            order_details = result.get("data", {})
            # خصم الرصيد محلياً وتحديث السجلات
            # (نعتمد على الخصم التلقائي من API المزود، ولكن نقوم بتحديث رصيد المستخدم المحلي بناءً على تكلفة المنتج إن وجد)
            bot.send_message(chat_id, f"🎉 **تم شراء المنتج وتسليمه بنجاح!**\n\n📦 تفاصيل الطلب:\n`{order_details}`", parse_mode="Markdown", reply_markup=get_back_to_store_keyboard())
        else:
            err_msg = result.get("error", "حدث خطأ غير معروف أثناء الشراء.")
            bot.send_message(chat_id, f"❌ **فشل اتمام الطلب التلقائي:**\n{err_msg}", parse_mode="Markdown", reply_markup=get_back_to_store_keyboard())
        return
    ...
    # ==================== عروض Telegram (السيرفرات ثم الدول) ====================
    ...
    # ==================== حسابات تيليجرام جاهزة ====================
        ...
        parts = call.data.split("_")
        srv_num, c_name, price, count = parts[2], parts[3], float(parts[4]), parts[5]

        detail_msg = (
            f"💙 **حساب Telegram جاهز - نقل كود 100%**\n"
            f"---------------------------\n"
            f"▫️ **الدولة** : {c_name}\n"
            f"▫️ **السعر** : ${price:.2f}\n"
            f"▫️ **متوفر** : {count} الرقم\n\n"
            f"**ملاحظة**: هذة الخدمة تعطيك رقم تم تجهيزة مسبقا وكل ماعليك نقل الرقم فقط فلا يمكنك إلغاء الرقم بعد شرائة وصول الكود مضمون 100%"
        )
        markup = ready_account_detail_keyboard(srv_num, c_name, price)
        try:
        ...
        parts = call.data.split("_")
        srv_num, c_name, cost = parts[3], parts[4], float(parts[5])
        conn = get_db()
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT balance FROM users WHERE user_id = ?", (user_id,))
        ...
        success_ready = (
            ...
            f"📩 تم تسليم بيانات الحساب لخاصك بنجاح."
        )
