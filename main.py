...
@bot.message_handler(func=lambda msg: msg.from_user.id in USER_STEPS, content_types=['text'])
def handle_user_steps(message):
    ...
    if str(user_id) == str(ADMIN_ID):
        if step == 'ADMIN_BROADCAST':
            ...
            try:
                ...
                bot.send_message(message.chat.id, "❌ يرجى إرسال رقم صحيح للمبلغ:", reply_markup=admin_back_button())
                return
            conn = get_db()
            cursor = conn.cursor()
    ...
    if step == 'TRANSFER_TARGET':
        ...
        step_data['link'] = message.text.strip()
