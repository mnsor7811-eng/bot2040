...
# ==================== حل مشكلة تعارض الويب هوك وتعيين الأوامر ====================
try:
    bot.remove_webhook()
    logger.info("تم حذف الويب هوك بنجاح")
except Exception as e:
    logger.warning(f"تنبيه الويب هوك: {e}")

try:
    bot.set_my_commands([
        BotCommand("start", "بدء تشغيل البوت وفتح القائمة الرئيسية"),
        BotCommand("help", "الحصول على المساعدة والدعم الفني"),
        BotCommand("account", "عرض بيانات حسابك والرصيد"),
...
def get_smm_services(server_id='2'):
    ...
    try:
        ...
        logger.error(f"خطأ في جلب خدمات الرشق: {e}")
    return SMM_SERVICES_CACHE.get(server_id, [])

# ==================== التحقق من الاشتراك الإجباري للقنوات ====================
def check_channel_sub(user_id):
    # في حال الرغبة بالتحقق الصارم من عضوية القناة الرسمية
    if not CHANNEL_OFFICIAL_ID or CHANNEL_OFFICIAL_ID == "":
        return True
    try:
        chat_member = bot.get_chat_member(f"-100{CHANNEL_OFFICIAL_ID}", user_id)
        if chat_member.status in ['member', 'administrator', 'creator']:
            return True
        return False
    except Exception:
        # إذا لم يكن البوت مشرفاً في القناة يتم التمرير بدون تعطيل المستخدم
        return True
...
def help_command(message):
    help_text = (
        "💡 **دليل استخدام البوت:**\n\n"
        "1️⃣ **شراء الأرقام الوهمية:** اختر السيرفر ثم التطبيق ثم الدولة، وسيصلك الكود فوراً داخل البوت.\n"
        "2️⃣ **حسابات تيليجرام الجاهزة:** يتم تسليم الحسابات المجهزة والجاهزة فور الشراء مع ملف الجلسة.\n"
        "3️⃣ **شحن الرصيد:** يدعم البوت الكريمي، جيب، ون كاش، والدفع التلقائي الفوري عبر Binance Pay.\n"
        "4️⃣ **الرشق وشحن الألعاب:** تنفيذ طلبات المتابعين، الشدات، واللايكات بأفضل الأسعار.\n"
        "5️⃣ **نظام الإحالة:** اربح رصيد حقيقي عن كل صديق تقوم بدعوته للبوت.\n\n"
        f"لأي استفسار يمكنك التواصل مع الدعم: {ADMIN_USERNAME}"
    )
    bot.send_message(message.chat.id, help_text, reply_markup=back_button(), parse_mode="Markdown")

# ==================== أمر الحساب /account ====================
@bot.message_handler(commands=['account'])
def account_command(message):
    user_id = message.from_user.id
    user = get_or_create_user(user_id, message.from_user.first_name or "")
    balance = user[3]
    ...
    bot.send_message(message.chat.id, acc_text, reply_markup=back_button(), parse_mode="Markdown")
...
# ==================== معالجة استفسارات الذكاء الاصطناعي العامة ====================
@bot.message_handler(func=lambda msg: msg.from_user.id not in USER_STEPS and msg.text and msg.text.startswith("🤖"))
def handle_ai_query(message):
    ...
    row = cursor.fetchone()
    ai_bal = row[0] if row else 0

    if ai_bal <= 0:
        conn.close()
        bot.reply_to(message, "⚠️ نفد رصيد أسئلة الذكاء الاصطناعي المجاني لديك. يمكنك شحن رصيد الأسئلة من القائمة أو التواصل مع الإدارة.")
        return

    query = message.text.replace("🤖", "").strip()
    if not query:
        conn.close()
        bot.reply_to(message, "✍️ يرجى كتابة سؤالك بعد رمز 🤖 وسأقوم بالإجابة عليك فوراً.")
        return

    waiting_msg = bot.reply_to(message, "🤖 جاري التفكير وصياغة الإجابة بواسطة الذكاء الاصطناعي...")
    try:
        if ai_model:
            response = ai_model.generate_content(query)
            ans_text = response.text
        else:
            ans_text = "نموذج الذكاء الاصطناعي قيد التحديث حالياً، يرجى المحاولة لاحقاً."

        cursor.execute("UPDATE users SET ai_balance = ai_balance - 1 WHERE user_id = ?", (user_id,))
        conn.commit()
        conn.close()

        bot.edit_message_text(f"🤖 **إجابة المساعد الذكي:**\n\n{ans_text}", chat_id=message.chat.id, message_id=waiting_msg.message_id, parse_mode="Markdown")
    except Exception as e:
        conn.close()
        bot.edit_message_text(f"❌ حدث خطأ أثناء معالجة السؤال: {e}", chat_id=message.chat.id, message_id=waiting_msg.message_id)

# ==================== معالجة الرسائل النصية والخطوات الموجهة ====================
...
def handle_user_steps(message):
    ...
    if step == 'binance_enter_amount':
        try:
            ...
            bot.send_message(message.chat.id, details_text, reply_markup=binance_details_keyboard(amount), parse_mode="Markdown")
        except ValueError:
            bot.reply_to(message, "⚠️ يرجى إدخال رقم صحيح للمبلغ (مثال: 5 أو 10.5):", reply_markup=binance_amount_keyboard())

    # 2. إدخال TXID بايننس والتحقق
    ...
    # 4. إدخال كمية الرشق
        ...
        try:
            ...
            USER_STEPS[user_id] = {
                'step': 'smm_ready_to_confirm',
                'srv_id': srv_id,
                'quantity': qty,
                'total_cost': total_price,
                'link': link,
                'service_name': target_srv.get('name', 'خدمة') if target_srv else srv_id
            }
            ...
            bot.send_message(message.chat.id, confirm_text, reply_markup=smm_confirm_keyboard(), parse_mode="Markdown")
        except ValueError:
            bot.reply_to(message, "⚠️ يرجى إدخال عدد صحيح للكمية.")

    # 5. شحن رصيد ذاتي للأدمن
    ...
    # 6. إضافة رصيد لمستخدم بواسطة الأدمن
...
def handle_callbacks(call):
    ...
    if data.startswith("ready_server_"):
        ...
        countries = fetch_ready_accounts_api(srv_id)
        if countries:
            text = f"🌍 الدول المتاحة للحسابات الجاهزة (السيرفر {srv_id}) - تم جلبها مباشرة من المزود:"
        else:
            text = f"🌍 السيرفر {srv_id}:\n⚠️ لا توجد حسابات متوفرة حالياً من مزود السيرفر أو لم يرجع الـ API أي بيانات متاحة."
    ...
    if data.startswith("bin_amt_"):
        amt_str = data.replace("bin_amt_", "")
        if amt_str == "custom":
            USER_STEPS[user_id] = {'step': 'binance_enter_amount'}
            bot.edit_message_text("✍️ أرسل المبلغ المطلوب شحنه بالدولار (مثال: 15.5):", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=binance_amount_keyboard())
        else:
            ...
            bot.edit_message_text(details_text, chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=binance_details_keyboard(amount), parse_mode="Markdown")
        return

    if data.startswith("bin_paid_"):
        amount = float(data.replace("bin_paid_", ""))
        USER_STEPS[user_id] = {'step': 'binance_awaiting_txid_input', 'amount': amount}
    ...
    # ==================== قسم شراء الأرقام ====================
    if data == "buy_number":
        bot.edit_message_text("📞 **سيرفرات الأرقام الوهمية:**\nاختر السيرفر المفضل:", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=servers_keyboard(), parse_mode="Markdown")
        return
    ...
    if data.startswith("buy_"):
        ...
        cost = float(parts[4]) if len(parts) > 4 else DEFAULT_PRICE
    ...
    if data == "smm_boost":
        bot.edit_message_text("⭐ **اختر نوع الدعم المطلوب:**", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=boost_keyboard(), parse_mode="Markdown")
        return

    if data == "smm_srv_2":
        services = get_smm_services('2')
        categories = list(set([s.get('category', 'عام') for s in services if s.get('category')]))
        bot.edit_message_text("📁 **اختر القسم المطلوب:**", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=dynamic_smm_keyboard(categories, '2', page=0), parse_mode="Markdown")
        return

    if data.startswith("smmpage_"):
        parts = data.split("_")
        srv_id = parts[1]
        page = int(parts[2])
        services = get_smm_services(srv_id)
        categories = list(set([s.get('category', 'عام') for s in services if s.get('category')]))
        bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=dynamic_smm_keyboard(categories, srv_id, page=page))
        return
    ...
    if data.startswith("smm_cat_"):
        ...
        cat_name = parts[3]
        services = get_smm_services(srv_id)
        matched_services = [s for s in services if cat_name.lower() in str(s.get('category', '')).lower()]
        bot.edit_message_text(f"📋 **خدمات القسم:**", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=smm_detail_grid_keyboard(matched_services, srv_id, cat_name, page=0), parse_mode="Markdown")
        return

    if data.startswith("smmcatpage_"):
        parts = data.split("_")
        srv_id = parts[1]
        page = int(parts[2])
        services = get_smm_services(srv_id)
        bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=smm_detail_grid_keyboard(services, srv_id, "عام", page=page))
        return

    if data.startswith("smm_choose_"):
        parts = data.split("_")
        srv_id = parts[2]
        service_id = parts[3]
        services = get_smm_services(srv_id)
        target = next((s for s in services if str(s.get('service')) == str(service_id)), None)
        
        if target:
            USER_STEPS[user_id] = {'step': 'smm_awaiting_link', 'srv_id': srv_id, 'cat_code': target.get('category', 'others')}
            detail_txt = (
                f"📋 **بيانات الخدمة:**\n"
                f"🔹 الاسم: {translate_text(target.get('name', ''))}\n"
                f"🔹 السعر لكل 1000: `${round(float(target.get('rate', 1.0)) * 1.10, 4)}`\n"
                f"🔹 الحد الأدنى: `{target.get('min', '10')}`\n"
                f"🔹 الحد الأقصى: `{target.get('max', '10000')}`\n\n"
                f"✍️ **أرسل الآن رابط الحساب أو المنشور المطلوب:**"
            )
            bot.edit_message_text(detail_txt, chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=smm_cancel_link_keyboard(), parse_mode="Markdown")
        return

    if data == "smm_confirm_yes":
        step_info = USER_STEPS.get(user_id, {})
        srv_id = step_info.get('srv_id')
        qty = step_info.get('quantity', 0)
        total_cost = step_info.get('total_cost', 0.0)
        link = step_info.get('link', '')
        service_name = step_info.get('service_name', 'خدمة')
        ...
        if bal < total_cost:
            conn.close()
            bot.answer_callback_query(call.id, f"❌ رصيدك غير كافٍ لإتمام الطلب. التكلفة: {total_cost:.4f}$", show_alert=True)
            return

        srv_info = SMM_SERVERS.get(srv_id, {})
        order_res_id = f"ORD-{int(time.time())}"
        try:
            r = requests.post(srv_info['url'], data={'key': srv_info['key'], 'action': 'add', 'service': srv_id, 'link': link, 'quantity': qty}, timeout=10)
            res_data = r.json()
        ...
        cursor.execute("INSERT INTO smm_orders (order_id, user_id, service_id, service_name, quantity, cost, link) VALUES (?, ?, ?, ?, ?, ?, ?)",
                       (order_res_id, user_id, srv_id, service_name, qty, total_cost, link))
        conn.commit()
        conn.close()
        ...
        bot.edit_message_text(success_text, chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=smm_order_status_keyboard(order_res_id), parse_mode="Markdown")
        return

    if data.startswith("smm_chk_"):
        order_id = data.replace("smm_chk_", "")
        srv_info = SMM_SERVERS.get('2', {})
        status_txt = "قيد المعالجة (In Progress)"
        try:
            r = requests.post(srv_info['url'], data={'key': srv_info['key'], 'action': 'status', 'order': order_id}, timeout=8)
            res_data = r.json()
            if res_data.get('status'):
                status_txt = str(res_data.get('status'))
        except:
            pass
        bot.answer_callback_query(call.id, f"حالة الطلب {order_id}: {status_txt}", show_alert=True)
        return
...
# ==================== بدء تشغيل البوت مع إعادة المحاولة التلقائية ====================
if __name__ == '__main__':
    logger.info("🤖 تم بدء تشغيل البوت بنجاح وحفظ البيانات بشكل دائم في SQLite...")
    while True:
        try:
