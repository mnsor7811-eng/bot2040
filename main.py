...
# ==================== إعدادات API موقع SMMStone (محدث) ====================
SMM_API_KEY = "01e140e9a06bbce1c5633971a354cf77"
...
def callback_handler(call):
    ...
    # ==================== تعديل اخفاء اسم السيرفر الأصلي ====================
    elif call.data.startswith("select_server_"):
        server_id = call.data.split("_")[2]
        # تحويل المعرف الأصلي إلى اسم مخصص مشفر للعميل (مثال: سيرفر الأرقام 1)
    ...
    # ==================== أقسام الرشق الشفافة التفاعلية (SMM Stone) ====================
...
def handle_user_steps(message):
    ...
    if step == 'TRANSFER_TARGET':
        ...
        if not target_user:
            bot.send_message(message.chat.id, "❌ هذا المستدم غير موجود في البوت.")
            return
...
# ==================== استقبال واستجابة الذكاء الاصطناعي (مع تحسين الخصم) ====================
...
def handle_ai_chat(message):
    ...
    if res and res[0] > 0:
        try:
            ...
            # الخصم بعد النجاح لضمان عدم ضياع المحاولات
            cursor.execute("UPDATE users SET ai_balance = ai_balance - 1 WHERE user_id=?", (user_id,))
            conn.commit()
        except Exception as e:
            print(f"AI Error: {e}")
            bot.reply_to(message, "⚠️ خطأ في الاتصال بالذكاء الاصطناعي. لم يتم خصم أي رصيد من حسابك.")
        finally:
            conn.close()
    else:
        conn.close()
        bot.reply_to(message, "❌ نفد رصيد أسئلة الذكاء الاصطناعي الخاص بك.")

set_bot_commands()
print("Bot is running smoothly with Full Menu Commands & Hidden Server Details...")
