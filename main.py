...
# ==================== 2. قاعدة البيانات (تم التعديل لإضافة timeout) ====================
def setup_db():
...
def get_db():
    # إضافة timeout لمنع خطأ database is locked عند الضغط المتكرر
...
def boost_keyboard():
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("📱 Telegram", callback_data="boost_tg"),
        InlineKeyboardButton("📸 Instagram", callback_data="boost_ig"),
        InlineKeyboardButton("🎵 Tik Tok", callback_data="boost_tk"),
        InlineKeyboardButton("📘 Facebook", callback_data="boost_fb"),
        InlineKeyboardButton("🐦 Twitter X", callback_data="boost_tw"),
        InlineKeyboardButton("▶️ YouTube", callback_data="boost_yt"),
        InlineKeyboardButton("🟢 Whatsapp", callback_data="boost_wa"),
        InlineKeyboardButton("🍊 KWAI", callback_data="boost_kwai"),
        InlineKeyboardButton("🟡 SNAPCHAT", callback_data="boost_sc"),
        InlineKeyboardButton("🟢 Free services", callback_data="boost_free"),
        ...
        if purchase and purchase[2] == 'PENDING':
            ...
            if action_type == "change_num":
                ...
                if "ACCESS_NUMBER" in res:
                    ...
                    formatted_phone = f"+{new_raw_phone}" if not raw_phone.startswith("+") else raw_phone
                    c_name, c_flag = get_clean_country_info(country_code)
...
# ==================== ربط لوحة الإدارة (النهاية) ====================
admin.register_admin_handlers(bot, ADMIN_ID, get_db)
