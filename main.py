...
from keyboards import (
    ...
    more_settings_keyboard, ready_accounts_keyboard, ready_accounts_countries_keyboard,
    tg_servers_keyboard, recharge_keyboard, servers_keyboard, services_keyboard,
    countries_keyboard_fast, active_number_keyboard, smm_main_keyboard, games_keyboard,
    boost_keyboard, dynamic_smm_keyboard, smm_detail_grid_keyboard, smm_cancel_link_keyboard,
...
def callback_handler(call):
    ...
    # ==================== 1. زر الإعدادات والمزيد ====================
    if call.data == "more_settings_menu":
        try: bot.answer_callback_query(call.id)
        except: pass
    ...
    # ==================== 2. زر عروض Telegram (السيرفرات ثم الدول) ====================
    ...
    # ==================== 3. زر حسابات تيليجرام جاهزة ====================
    elif call.data == "ready_accounts_menu":
        try: bot.answer_callback_query(call.id)
        except: pass
        ready_text = (
            "💯 **قسم حسابات تيليجرام الجاهزة**\n\n"
            "▫️ حسابات مفعلة بجلسات Tdata و Session.\n"
            "▫️ تسليم فوري وتلقائي مع ضمان الاستلام.\n"
            "▫️ مربوطة بمزودي TG-Lion و Spider TG بنسبة ربح 10%.\n\n"
        ...
        srv_name = "TG-Lion API" if srv_num == "1" else "Spider TG API"
        markup = ready_accounts_countries_keyboard(server_id=srv_num, page=0)
        msg_txt = f"💯 **الحسابات المتاحة عبر {srv_name}**:\n\nاختر الدولة المطلوبة:"
        try:
        ...
        srv_num, page = parts[1], int(parts[2])
        srv_name = "TG-Lion API" if srv_num == "1" else "Spider TG API"
        markup = ready_accounts_countries_keyboard(server_id=srv_num, page=page)
        try:
            bot.edit_message_text(f"💯 **الحسابات المتاحة عبر {srv_name}**:\n\nاختر الدولة المطلوبة:", chat_id, message_id, parse_mode="Markdown", reply_markup=markup)
        except:
            pass
        return

    elif call.data.startswith("buy_ready_"):
        parts = call.data.split("_")
        srv_num, c_name, cost = parts[2], parts[3], float(parts[4])
        conn = get_db()
        cursor = conn.cursor()
        try:
            ...
            if bal < cost:
                bot.answer_callback_query(call.id, f"❌ رصيدك غير كافٍ! سعر الحساب: ${cost:.2f}\nرصيدك: ${bal:.2f}", show_alert=True)
                return
            cursor.execute("UPDATE users SET balance = balance - ? WHERE user_id = ?", (cost, user_id))
            cursor.execute("INSERT INTO ready_accounts_orders (user_id, server_id, country_name, cost) VALUES (?, ?, ?, ?)",
                           (user_id, srv_num, c_name, cost))
            conn.commit()
        finally:
            conn.close()

        success_ready = (
            f"🎉 **تم شراء حساب تيليجرام جاهز بنجاح!**\n\n"
            f"🌐 الدولة: {c_name}\n"
            f"💵 التكلفة المخصومة: `${cost:.2f}`\n"
            f"📁 نوع الملف: `Tdata / Session`\n"
            f"🔑 كود الطلب: `TG-{int(time.time())}`\n\n"
            f"📩 تم تجهيز الحساب وتسليمه بنجاح."
        )
        try: bot.edit_message_text(success_ready, chat_id, message_id, parse_mode="Markdown", reply_markup=back_button())
        except: bot.send_message(chat_id, success_ready, parse_mode="Markdown", reply_markup=back_button())
        return

    # ==================== لوحة الإدارة الكبرى ====================
    elif call.data == "admin_panel":
        if user_id != ADMIN_ID:
            bot.answer_callback_query(call.id, f"❌ هذه اللوحة للمشرف فقط. آيديك: {user_id}", show_alert=True)
            return
    ...
    elif call.data.startswith("admin_self_charge_"):
        ...
        try:
            ...
            conn.close()
        bot.answer_callback_query(call.id, f"✅ تم شحن ${amount} لحسابك بنجاح! رصيدك الآن: ${new_bal:.2f}", show_alert=True)
        return
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT user_id, name, username, balance, is_banned FROM users ORDER BY rowid DESC LIMIT 10")
        ...
        if not users:
            ...
            return
        bot.send_message(chat_id, "📋 أحدث 10 مستخدمين مسجلين في البوت:")
        for u in users:
    ...
    # ==================== باقي معالجات الأزرار ====================
        ...
        bot.send_message(chat_id, f"🟡 **شحن رصيد عبر Binance**\n\n💵 ارسل المبلغ الذي تريد شحنه (بالدولار USDT):", reply_markup=markup, parse_mode="Markdown")
        ...
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("🔙 رجوع", callback_data="recharge_menu"))
        bot.send_message(chat_id, "🟡 أدخل رقم المعاملة (TXID):", reply_markup=markup)
        if user_id in USER_STEPS: USER_STEPS[user_id]["step"] = "waiting_binance_txid"
        else: USER_STEPS[user_id] = {"step": "waiting_binance_txid"}

    elif call.data == "copy_id":
        bot.answer_callback_query(call.id, text=f"Pay ID: {BINANCE_PAY_ID}")
    ...
    # ==================== أقسام الرشق (SMM) ====================
        ...
        bot.edit_message_text("🔥 أكثر السيرفرات طلباً: Grizzly SMS و SMMXstar", chat_id, message_id, reply_markup=back_button())
...
def handle_user_steps(message):
    ...
    if step == "waiting_binance_amount":
        try:
            ...
            markup.add(InlineKeyboardButton("🔙 رجوع", callback_data="recharge_menu"))
            bot.send_message(message.chat.id, f"🟡 **تفاصيل الدفع عبر Binance**\n\n💵 المبلغ: {amount} USDT\n📍 عنوان (Pay ID):\n`{BINANCE_PAY_ID}`", reply_markup=markup, parse_mode="Markdown")
        except:
            bot.send_message(message.chat.id, "❌ يرجى إرسال رقم صحيح للمبلغ:")
        return
    ...
    if user_id == ADMIN_ID:
        if step == 'ADMIN_BROADCAST':
            ...
            try:
                t_id, amt = int(message.text.split()[0]), float(message.text.split()[1])
                conn = get_db()
                cursor = conn.cursor()
                try:
                    ...
                    conn.close()
                del USER_STEPS[user_id]
                bot.send_message(message.chat.id, f"✅ تم إضافة ${amt:.2f} للحساب `{t_id}`.", parse_mode="Markdown", reply_markup=admin_back_button())
            except:
                bot.send_message(message.chat.id, "❌ الصيغة: `ID المبلغ`", parse_mode="Markdown")
            return
            ...
            try:
                ...
                try:
                    ...
                    conn.close()
                del USER_STEPS[user_id]
                bot.send_message(message.chat.id, f"✅ تم خصم ${amt:.2f} من الحساب `{t_id}`.", parse_mode="Markdown", reply_markup=admin_back_button())
            except:
                bot.send_message(message.chat.id, "❌ الصيغة: `ID المبلغ`", parse_mode="Markdown")
            return
            ...
            try:
                ...
                cursor = conn.cursor()
                try:
                    if step == 'ADMIN_ADD_BALANCE_DIRECT': cursor.execute("UPDATE users SET balance = balance + ? WHERE user_id = ?", (val, t_id))
                    else: cursor.execute("UPDATE users SET balance = balance - ? WHERE user_id = ?", (val, t_id))
                    conn.commit()
                finally:
                    conn.close()
                del USER_STEPS[user_id]
                bot.send_message(message.chat.id, f"✅ تمت العملية بنجاح للحساب `{t_id}`.", parse_mode="Markdown", reply_markup=admin_back_button())
            except:
                bot.send_message(message.chat.id, "❌ أدخل قيمة صحيحة.")
            return

    # --- استلام رابط ثم كمية الرشق ---
