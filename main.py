...
def get_main_welcome_text(user_id, name="", username="", balance=0.0):
    ...
    # حساب المسافات ديناميكياً لدفع بيانات المستخدم (اليوزر، الآيدي، الرصيد) إلى أقصى اليسار تماماً
    target_width = 50
    ch1_len = 20  # 'قناة البوت الرسمية 📎'
    ch2_len = 16  # 'قناة التفعيلات 🛒'
    ch3_len = 20  # 'قناة شرح الاستخدام 📚'

    u1_len = len(f"💙 {raw_disp} 💙")
    u2_len = len(f"👤 : {user_id} •")
    u3_len = len(f"💲 : {bal_str}$ •")

    sp1 = " " * max(3, target_width - ch1_len - u1_len)
