...
def callback_handler(call):
    ...
    # ==================== أقسام الرشق (SMM) ====================
        ...
        cursor = conn.cursor()
        try:
            cursor.execute('SELECT balance, spent_balance, orders_count, ai_balance FROM users WHERE user_id = ?', (user_id,))
            ...
            ai_bal = urow[3] if urow and urow[3] is not None else 5
        finally:
            conn.close()
        ...
        msg = (
            ...
            f"🤖 <b>رصيد الذكاء الاصطناعي</b> : <b>{ai_bal}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━"
        )
