...
def get_main_welcome_text(user_id, name="", username="", balance=0.0):
    ...
    # أسماء القنوات حسب طلب المستخدم (الإيموجي يبدأ من اليمين، والأسماء دقيقة ومختصرة):
    ch1_name = "📎 قناة البوت"
    ...
    # عرض متوازن للسطر (36 حرفاً) ليتوافق مع شاشات الجوال بدون أي كسر أو التفاف للسطر
    target_width = 36
    sp1 = " " * max(2, target_width - len(ch1_name) - len(bal_part))
    sp2 = " " * max(2, target_width - len(ch2_name) - len(id_vis))
    sp3 = " " * max(2, target_width - len(ch3_name) - len(usr_vis))

    line1 = f"{rlm}<a href=\"{ch_off}\">{ch1_name}</a>{sp1}{ltr}{bal_part}{ltr}"
    line2 = f"{rlm}<a href=\"{ch_ord}\">{ch2_name}</a>{sp2}{ltr}{id_part}{ltr}"
    line3 = f"{rlm}<a href=\"{ch_tut}\">{ch3_name}</a>{sp3}{ltr}{usr_part}{ltr}"
    line4 = f"{rlm}<a href=\"{ch_exp}\">{ch4_name}</a>"
