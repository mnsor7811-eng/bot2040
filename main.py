...
def callback_handler(call):
    ...
    if call.data == "admin_panel":
        ...
        markup.row(InlineKeyboardButton("💰 إضافة رصيد", callback_data="admin_add_balance"), InlineKeyboardButton("➖ خصم رصيد", callback_data="admin_deduct_balance"))
        ...
        bot.send_message(chat_id, "🔍 أرسل اسم المستخدم (أو جزء منه) أو الآيدي الخاص به للبحث عنه:", reply_markup=admin_back_button())
        return
...
def handle_user_steps(message):
    ...
    if str(user_id) == str(ADMIN_ID):
        if step == 'ADMIN_BROADCAST':
            ...
            conn = get_db()
            cursor = conn.cursor()
            cursor.execute("SELECT user_id, name, balance, ai_balance, is_banned FROM users WHERE name LIKE ? OR user_id = ?", ('%' + query + '%', query))
            users = cursor.fetchall()
            conn.close()

            if not users:
                bot.send_message(message.chat.id, "❌ لم يتم العثور على أي مستخدم بهذا الاسم أو الآيدي.", reply_markup=admin_back_button())
                return

            res = "🔍 **نتائج البحث عن المستخدمين:**\n\n"
            for u in users:
                ...
                res += (f"👤 **الاسم**: {u[1]}\n"
                        ...
                        f"📌 **الحالة**: {status}\n"
                        f"-------------------------\n")
            bot.send_message(message.chat.id, res, parse_mode="Markdown", reply_markup=admin_back_button())
            return
            ...
            conn = get_db()
            cursor = conn.cursor()
            cursor.execute("SELECT name, balance FROM users WHERE user_id = ?", (target_id,))
            target = cursor.fetchone()
            if not target:
                bot.send_message(message.chat.id, "❌ المستخدم غير موجود في قاعدة البيانات.", reply_markup=admin_back_button())
                conn.close()
            ...
            try:
                bot.send_message(target_id, f"⚠️ قامت الإدارة بخصم `${amount:.2f}` من رصيدك.")
            except:
                pass
            return
            ...
            target = cursor.fetchone()
            if not target:
                bot.send_message(message.chat.id, "❌ المستخدم غير موجود.", reply_markup=admin_back_button())
                conn.close()
                return
            ...
            try:
                bot.send_message(target_id, f"🎁 قامت الإدارة بإضافة {count} سؤال ذكاء اصطناعي إلى حسابك!")
            except:
                pass
            return
