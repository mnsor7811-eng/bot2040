...
def get_main_welcome_text(user_id, name="", username="", balance=0.0):
    ...
    if clean_uname and clean_uname.lower() not in ['none', 'لا يوجد', '']:
        user_disp = clean_uname if clean_uname.startswith('@') else f"@{clean_uname}"
    else:
        user_disp = str(name or "المستخدم").strip()
    ...
    return (
        ...
        f"👤 : <code>{user_id}</code> •\n"
        f"💲 : {bal_str}$ •\n\n"
        f"                        <a href=\"{ch_off}\">قناة البوت الرسمية 📎</a>\n"
        f"                            <a href=\"{ch_ord}\">قناة التفعيلات 🛒</a>\n"
        f"                      <a href=\"{ch_tut}\">قناة شرح الاستخدام 📚</a>\n"
        f"                  <a href=\"{ch_exp}\">قناة الشروحات والإرشادات 🎬</a>\n\n"
...
def callback_handler(call):
    ...
    # ==================== باقي معالجات الأزرار القياسية ====================
        ...
        text = get_main_welcome_text(user_id, call.from_user.first_name, user_display, balance)
