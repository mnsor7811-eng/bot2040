...
def callback_handler(call):
    ...
    # ==================== أقسام الرشق (SMM) ====================
        ...
        srv_name = "سيرفر اسمم اكسترا"
        try:
            bot.edit_message_text(f"🐙 **{srv_name}**\n\nتوفر خدمات متابعين وإعجابات ومشاهدات بأسعار مناسبة\n\n🧛‍♂️ الرجاء إختيار الخدمة:", chat_id, message_id, parse_mode="Markdown", reply_markup=boost_keyboard("2"))
        except Exception:
            bot.send_message(chat_id, f"🐙 **{srv_name}**\n\nتوفر خدمات متابعين وإعجابات ومشاهدات بأسعار مناسبة\n\n🧛‍♂️ الرجاء إختيار الخدمة:", parse_mode="Markdown", reply_markup=boost_keyboard("2"))
        ...
        if call.data.startswith("smmbuy_"):
            ...
            USER_STEPS[user_id] = {'step': 'WAITING_LINK', 'service_id': service_id, 'server_id': '2'}
...
def handle_user_steps(message):
    ...
    if step == 'WAITING_LINK':
        ...
        if str(service_id).startswith("game_"):
            ...
            markup = smm_confirm_keyboard(service_id, qty, total_cost, smm_server_id=server_id)
            bot.send_message(
                message.chat.id,
        ...
        markup = smm_confirm_keyboard(service_id, qty, total_cost_with_profit, smm_server_id=server_id)
        bot.send_message(
            message.chat.id,
            f"📋 **تأكيد تفاصيل الطلب:**\n\n"
            f"🔗 الرابط: `{link}`\n"
