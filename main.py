...
# ==================== إعدادات مزودي خدمات الرشق المتعددة (SMM Providers) ====================
SMM_PROVIDERS = {
    "1": {
        "name": "السيرفر الأول (عربي - SMMXStar)",
        "url": "https://smmxstar.com/api/v2",
        "key": "f69d573e51807b598bc38682884dc8f6"
    },
    "2": {
        "name": "السيرفر الثاني (SMMStone)",
        "url": "https://smmstone.com/api/v2",
        "key": "01e140e9a06bbce1c5633971a354cf77"
    },
    "3": {
        "name": "السيرفر الثالث (SMMFollows)",
        "url": "https://smmfollows.com/api/v2",
        "key": "254a2bcd0869ffc5d361dec2298d3691"
    }
}
...
def smm_request(provider_id, action, **kwargs):
    """دالة موحدة للاتصال بأي سيرفر حسب provider_id لمنع التعارض بالكامل"""
    provider = SMM_PROVIDERS.get(str(provider_id))
    if not provider:
        return None
        
    payload = {'key': provider['key'], 'action': action}
    payload.update(kwargs)
    try:
        response = requests.post(provider['url'], data=payload, timeout=12)
        return response.json()
    except Exception as e:
        print(f"SMM API Error (Provider {provider_id}): {e}")
        return None

def get_cached_smm_services(provider_id):
    global SMM_SERVICES_CACHE, SMM_CACHE_TIME
    pid = str(provider_id)
    
    current_time = time.time()
    if pid in SMM_SERVICES_CACHE and (current_time - SMM_CACHE_TIME.get(pid, 0) < 300):
        return SMM_SERVICES_CACHE[pid]
    
    data = smm_request(pid, 'services')
    if data and isinstance(data, list):
        SMM_SERVICES_CACHE[pid] = data
        SMM_CACHE_TIME[pid] = current_time
        return data
    return []

def filter_smm_services(provider_id, target_type):
    services = get_cached_smm_services(provider_id)
    filtered = []
...
def callback_handler(call):
    ...
    # ==================== أقسام الرشق والمتجر ====================
        ...
        bot.edit_message_text("🌐 اختر سيرفر الرشق والمتابعين المطلوب:", chat_id, message_id, reply_markup=smm_servers_select_keyboard())

    elif call.data.startswith("select_smm_prov_"):
        provider_id = call.data.split("_")[3]
        provider_name = SMM_PROVIDERS[provider_id]["name"]
        bot.edit_message_text(f"⚙️ تم اختيار: **{provider_name}**\n\nاختر نوع الخدمة التي تود طلبها:", chat_id, message_id, parse_mode="Markdown", reply_markup=boost_keyboard(provider_id))
        ...
        filtered_services = filter_smm_services(provider_id, category_code)
        
        if not filtered_services:
            bot.send_message(chat_id, "❌ عذراً، لا توجد خدمات متاحة لهذا القسم في هذا السيرفر حالياً.", reply_markup=back_button())
            return
            
        markup = dynamic_smm_keyboard(filtered_services, category_code, provider_id=provider_id, page=0)
        ...
        page = int(parts[3])
        filtered_services = filter_smm_services(provider_id, category_code)
        markup = dynamic_smm_keyboard(filtered_services, category_code, provider_id=provider_id, page=page)
        ...
        if call.data.startswith("smmbuy_"):
            ...
            msg_text = (f"▫️ اسم الخدمة : {name}\n"
                        f"▫️ الفئة : {selected_srv.get('category', 'عام')}\n"
                        f"▫️ وصف الخدمة :\n"
            ...
            bot.edit_message_text(msg_text, chat_id, message_id, reply_markup=grid_markup)
        else:
            service_id = call.data
            USER_STEPS[user_id] = {'step': 'WAITING_LINK', 'service_id': service_id, 'provider_id': 'game'}
        ...
        USER_STEPS[user_id] = {'step': 'WAITING_LINK', 'provider_id': provider_id, 'service_id': service_id}
        bot.send_message(chat_id, "🔗 أرسل الرابط أو المعرف (ID) المطلوب للخدمة الآن:", parse_mode="Markdown")
        ...
        if provider_id == "game" or str(service_id).startswith("game_"):
        ...
        order_response = smm_request(provider_id, 'add', service=service_id, link=link, quantity=qty)
        ...
        if order_response and 'order' in order_response:
            ...
            err_msg = order_response.get('error', 'خطأ غير معروف') if order_response else 'فشل الاتصال بالسيرفر'
...
def handle_user_steps(message):
    ...
    if step == 'WAITING_LINK':
        ...
        provider_id = step_data.get('provider_id', '1')
        link = step_data.get('link')
        
        if provider_id == "game" or str(service_id).startswith("game_"):
            total_cost = round((1.00 / 1000) * qty, 2)
            markup = smm_confirm_keyboard(provider_id, service_id, qty, total_cost)
            bot.send_message(
                message.chat.id,
        ...
        services_data = get_cached_smm_services(provider_id)
        service_rate = 0.0
        min_q = 1
        max_q = 1000000
        ...
        markup = smm_confirm_keyboard(provider_id, service_id, qty, total_cost_with_profit)
        bot.send_message(
            message.chat.id,
...
set_bot_commands()
print("Bot is running smoothly with Multi-SMM Providers...")
