...
def send_to_channel_safe(text, reply_markup=None):
    ...
    if ch_off_id and str(ch_off_id) not in targets:
        targets.append(str(ch_off_id))
        
    last_err = None
    for target in targets:
        # المحاولة 1: تنسيق HTML
        try:
            bot.send_message(target, text, parse_mode="HTML", reply_markup=reply_markup, disable_web_page_preview=True)
            return True, target
        except Exception as e:
            last_err = e
            # المحاولة 2: إرسال بدون parse_mode لتفادي أي خطأ في وسوم التنسيق
            try:
...
def notify_number_activation(order_id, country_name, country_flag, server_name, service_name, phone, code, price, user_id):
    ...
    text = (
        f"<b>🌟 Number-sms sales | التفعيلات والطلبات الناجحة</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🔔 <b>رقم الطلب</b> : <code>{html.escape(str(masked_p))}</code>\n"
        f"🌐 <b>الدولة</b> : {html.escape(str(country_name))} {country_flag}\n"
        f"👍 <b>التفعيل</b> : آلي / فوري ✅\n"
        f"🛍️ <b>السيرفر</b> : {html.escape(str(server_name))}\n"
        f"📱 <b>المنصة</b> : {html.escape(str(service_name))}\n"
        ...
        f"➕ <b>المرسل</b> : {html.escape(str(service_name))}\n"
        f"👁️ <b>كود التفعيل</b> : <code>{html.escape(str(masked_c))}</code>\n"
        f"━━━━━━━━━━━━━━━━━━━━"
    )
...
def notify_ready_account_activation(order_id, country_name, phone, code, password, price, user_id, server_name="سيرفر الحسابات 1"):
    ...
    text = (
        ...
        f"🌐 <b>الدولة</b> : {html.escape(str(country_name))}\n"
        f"📱 <b>المنصة</b> : تيليجرام (حساب جاهز ومفعل 100%)\n"
        f"📞 <b>الرقم</b> : <code>{html.escape(str(masked_p))}</code>\n"
        f"🆔 <b>العميل</b> : <code>{html.escape(str(masked_u))}</code>\n"
        f"💵 <b>السعر</b> : ${float(price):.2f}\n"
        f"⏰ <b>وقت التفعيل</b> : {now_str}\n"
        f"✅ <b>الحالة</b> : تم الدخول واستلام الكود بنجاح\n\n"
        f"🔑 <b>كود التفعيل</b> : <code>{html.escape(str(masked_c))}</code>\n"
...
def callback_handler(call):
    ...
    # ==================== حسابات تيليجرام جاهزة ====================
        ...
        try:
            ...
            try:
                bot.edit_message_text(f"⏳ **جاري معالجة شراء وحجز حساب {c_name}...**\nيرجى الانتظار بضع ثوانٍ...", chat_id, message_id, parse_mode="Markdown")
            except Exception:
                pass
            ...
            if not buy_result.get('ok'):
                ...
                try:
                    bot.edit_message_text(f"❌ **فشل إتمام الشراء من المزود:**\n{err_text}\n\n⚠️ لم يتم خصم أي مبلغ من رصيدك.", chat_id, message_id, parse_mode="Markdown", reply_markup=markup_back)
                except Exception:
                    bot.send_message(chat_id, f"❌ **فشل إتمام الشراء من المزود:**\n{err_text}\n\n⚠️ لم يتم خصم أي مبلغ من رصيدك.", parse_mode="Markdown", reply_markup=markup_back)
                return

            phone_number = buy_result.get('number')
            if not phone_number or str(phone_number).strip().lower() in ['none', 'null', 'غير محدد', '']:
                markup_back = ready_accounts_countries_keyboard(server_id=srv_id, age=age, page=0)
                err_msg = "❌ **لم يرجع المزود رقم هاتف صالح للحساب.**\n⚠️ تم إلغاء العملية تلقائياً ولم يتم خصم أي مبلغ من رصيدك."
                try:
                    bot.edit_message_text(err_msg, chat_id, message_id, parse_mode="Markdown", reply_markup=markup_back)
                except Exception:
                    bot.send_message(chat_id, err_msg, parse_mode="Markdown", reply_markup=markup_back)
                return
            ...
            try:
                bot.edit_message_text(success_msg, chat_id, message_id, parse_mode="Markdown", reply_markup=code_markup)
            except Exception:
                try:
