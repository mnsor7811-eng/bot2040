...
# ==================== تعيين قائمة الأوامر (Menu Commands) في تيليجرام ====================
def set_bot_commands():
    commands = [
        BotCommand("num", "🏡 القائمة الرئيسية"),
        BotCommand("whats", "🎁 عروض WhatsApp"),
        BotCommand("tele", "🎁 عروض Telegram"),
        BotCommand("telecu", "🎮 بيع/شراء أرقام Telegram"),
        BotCommand("severs", "💻 عرض جميع السيرفرات"),
        BotCommand("stock", "🚀 مخزون حسابات Telegram"),
        BotCommand("followers", "🧩 الرشق وزيادة المتابعين"),
        BotCommand("games", "🏓 شحن الألعاب والبرامج"),
        BotCommand("mart", "📊 إحصائيات الشراء الناجح لهذا اليوم"),
        BotCommand("language", "⚙️ لغة البوت")
    ]
    try:
...
# ==================== أوامر البدء والاختصارات السريعة (Menu Commands Handling) ====================
...
def tele_cmd(message):
    ...
    bot.send_message(message.chat.id, "🎁 عروض Telegram المتاحة:", reply_markup=markup)

@bot.message_handler(commands=['telecu'])
def telecu_cmd(message):
    bot.send_message(message.chat.id, "🎮 قسم بيع وشراء أرقام Telegram (قريباً أو اختر من السيرفرات المتاحة).", reply_markup=back_button())

@bot.message_handler(commands=['severs'])
def severs_cmd(message):
    bot.send_message(message.chat.id, "💻 اختر السيرفر المناسب لك لشراء الأرقام:", reply_markup=servers_keyboard())

@bot.message_handler(commands=['stock'])
def stock_cmd(message):
    bot.send_message(message.chat.id, "🚀 مخزون حسابات Telegram المتوفرة حالياً:", reply_markup=back_button())

@bot.message_handler(commands=['followers'])
def followers_cmd(message):
    bot.send_message(message.chat.id, "🧩 قسم الرشق وزيادة المتابعين:", reply_markup=smm_main_keyboard())

@bot.message_handler(commands=['games'])
def games_cmd(message):
    bot.send_message(message.chat.id, "🏓 قسم شحن الألعاب وبرامج بلاس:", reply_markup=games_keyboard())
...
def language_cmd(message):
    bot.send_message(message.chat.id, "⚙️ لغة البوت الحالية: اللغة العربية (اللغة الافتراضية).", reply_markup=back_button())
...
# تعيين القائمة عند بدء التشغيل
set_bot_commands()
print("Bot is running smoothly with Menu Commands...")
